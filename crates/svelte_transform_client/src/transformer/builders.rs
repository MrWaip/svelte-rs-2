...
impl<'a> ComponentTransformer<'_, 'a> {
    pub(crate) fn make_rune_get(&self, name: &str) -> Expression<'a> {
        rune_get(self.b, self.b.alloc_str(name))
    }

    pub(crate) fn make_rune_safe_get(&self, name: &str) -> Expression<'a> {
        rune_safe_get(self.b, self.b.alloc_str(name))
    }
    ...
    pub(crate) fn make_rune_set(
        ...
        rune_set(self.b, self.b.alloc_str(name), value, proxy)
    }
    ...
    pub(crate) fn make_rune_update(
        ...
        let name_arg = Argument::from(ast.expression_identifier(SPAN, self.b.alloc_str(name)));
    ...
    pub(crate) fn make_store_unsub(
        ...
        let ast = self.b.ast;
        let dollar_name = self.b.alloc_str(dollar_name);
        let callee = self.make_dollar_member("store_unsub");
        let inner_arg = Argument::from(inner);
        let label_arg = Argument::from(ast.expression_string_literal(SPAN, dollar_name, None));
        let stores_arg = Argument::from(ast.expression_identifier(SPAN, "$$stores"));
        ast.expression_call(
            SPAN,
            callee,
            NONE,
    ...
    pub(crate) fn make_untrack(&self, dollar_name: &str) -> Expression<'a> {
        untrack_ident(self.b, self.b.alloc_str(dollar_name))
    }
    ...
    pub(crate) fn each_item_collection_read_legacy(
        ...
        let hoisted = self
            ...
            .map(|s| self.b.alloc_str(s.as_str()));
    ...
    pub(crate) fn build_each_item_indexed_member_legacy(
        ...
        let property = match index_read {
            ...
            EachIndexStrategy::Direct => {
                ast.expression_identifier(SPAN, self.b.alloc_str(index_name))
            }
        };
    ...
    pub(crate) fn make_each_item_invalidate_seq(
            ...
            .find(|&sym| analysis.binding_semantics(sym).is_store())
        {
            let store_name = self
                .b
                .alloc_str(self.component_scoping.symbol_name(store_sym));
            let stores_arg = Argument::from(ast.expression_identifier(SPAN, "$$stores"));
            let label_arg = Argument::from(ast.expression_string_literal(SPAN, store_name, None));
    ...
    fn make_legacy_indirect_getter(
        ...
        self.b
            .ast
            .expression_identifier(SPAN, self.b.alloc_str(name))
    }
    ...
    pub(crate) fn make_dollar_member(&self, method: &'a str) -> Expression<'a> {
        let ast = self.b.ast;
    ...
    pub(crate) fn make_dollar_call(&self, method: &'a str, arg: Expression<'a>) -> Expression<'a> {
        let ast = self.b.ast;
