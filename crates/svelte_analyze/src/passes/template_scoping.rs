...
pub(crate) fn create_template_scopes(
    ...
    walk(&component.fragment, scoping, root, parsed);
}

fn walk(fragment: &Fragment, scoping: &mut ComponentScoping, current_scope: ScopeId, parsed: &ParserResult<'_>) {
    for node in &fragment.nodes {
        match node {
            Node::EachBlock(block) => {
                ...
                walk(&block.body, scoping, child_scope, parsed);
                if let Some(fb) = &block.fallback {
                    walk(fb, scoping, current_scope, parsed);
                }
            }
            Node::IfBlock(block) => {
                ...
                walk(&block.consequent, scoping, cons_scope, parsed);
                if let Some(alt) = &block.alternate {
                    ...
                    walk(alt, scoping, alt_scope, parsed);
                }
            }
            ...
            Node::AwaitBlock(block) => {
                if let Some(ref p) = block.pending {
                    ...
                    walk(p, scoping, scope, parsed);
                }
                if let Some(ref t) = block.then {
                    let scope = scoping.add_child_scope(current_scope);
                    scoping.set_fragment_scope(FragmentKey::AwaitThen(block.id), scope);
                    walk(t, scoping, scope, parsed);
                }
                if let Some(ref c) = block.catch {
                    let scope = scoping.add_child_scope(current_scope);
                    scoping.set_fragment_scope(FragmentKey::AwaitCatch(block.id), scope);
                    walk(c, scoping, scope, parsed);
                }
            }
            Node::Element(el) => walk(&el.fragment, scoping, current_scope, parsed),
            Node::ComponentNode(cn) => walk(&cn.fragment, scoping, current_scope, parsed),
            Node::ConstTag(_)
            | Node::SvelteWindow(_)
