...
fn gen_if_block<'a>(ctx: &mut Ctx<'a>, block_id: NodeId, anchor: Expression<'a>) -> Vec<Statement<'a>> {
    ...
    let alt_name: Option<String> = if has_alternate {
        let alt_key = alternate_key;
        let alt_name = ctx.gen_ident("alternate");
        let alt_body = gen_fragment(ctx, alt_key);
    ...
    // check whether alternate starts with an elseif block
    let alt_is_elseif = alt_name.as_ref().is_some_and(|_| {
