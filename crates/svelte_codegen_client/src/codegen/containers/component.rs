...
impl<'a, 'ctx> Codegen<'a, 'ctx> {
    pub(in crate::codegen) fn emit_component(
        ...
        let named_slots: Vec<(NodeId, FragmentKey)> =
            self.ctx.component_named_slots(el_id).to_vec();
        ...
        let children_key = FragmentKey::ComponentNode(el_id);
        let default_has_let = self.default_slot_has_let_directive_legacy(el_id);
        let children_body = if default_has_let {
            self.build_component_default_children_with_let(ctx, el_id, children_key)?
        } else {
            self.build_component_default_children(ctx, children_key)?
        };
        ...
        for (slot_el_id, frag_key) in named_slots {
            ...
            let arrow = self.build_named_slot_arrow(ctx, slot_el_id, frag_key)?;
