...
use svelte_analyze::{
    BindPropertyKind, DocumentBindKind, FragmentKey, ReferenceSemantics, WindowBindKind,
};
...
impl<'a, 'ctx> Codegen<'a, 'ctx> {
    ...
    fn emit_special_target_bind(
        ...
        if let Some(handle) = self
            ...
            .state
            .parsed
            .expr_handle(bind.expression_span.start)
        {
            let _ = self.ctx.state.parsed.take_expr(handle);
        }
    ...
    fn emit_svelte_head(
        ...
        let body_key = FragmentKey::SvelteHeadBody(el_id);
        let inner_ctx = ctx.child_of_svelte_head(body_key);
