...
impl<'a, 'ctx> Codegen<'a, 'ctx> {
    ...
    fn emit_element_html(
        ...
        if !self.ctx.query.view.is_void(el_id) {
            if self.ctx.is_customizable_select(el_id) {
                ...
                self.emit_fragment(state, &child_ctx, FragmentKey::Element(el_id))?;
    ...
    fn emit_customizable_select(
        ...
        self.emit_fragment(&mut inner_state, &child_ctx, FragmentKey::Element(el_id))?;
