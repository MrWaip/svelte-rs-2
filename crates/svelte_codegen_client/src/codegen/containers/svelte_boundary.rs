...
use svelte_analyze::{BlockSemantics, ConstTagBlockSemantics, FragmentKey};
...
impl<'a, 'ctx> Codegen<'a, 'ctx> {
    pub(in crate::codegen) fn emit_svelte_boundary(
        ...
        let attr_infos: Vec<(String, NodeId, bool, bool)> = boundary
            .attributes
            .iter()
            .filter_map(|attr| match attr {
                Attribute::ExpressionAttribute(a) => {
                    ...
                    Some((a.name.to_string(), a.id, is_dynamic, is_import))
                }
                _ => None,
            })
            .collect();
        ...
        for (name, attr_id, is_dynamic, is_import) in attr_infos {
        ...
        let body_key = FragmentKey::SvelteBoundaryBody(el_id);
        ...
        if !const_tag_ids.is_empty() && !snippet_children.is_empty() {
            for _ in 0..snippet_children.len() {
                ...
                for (cid, names) in &const_tag_names {
                    let Some(stmt_handle) = self.ctx.query.view.const_tag_stmt_handle(*cid) else {
                        continue;
                    };
                    if let Some(Statement::VariableDeclaration(decl)) =
                        self.ctx.state.parsed.stmt(stmt_handle)
                    {
        ...
        for (i, (snippet_id, _)) in snippet_children.iter().enumerate() {
            ...
            let snippet_uses_const = if !const_binding_syms.is_empty() {
                ...
                self.ctx
                    .fragment_references_any_symbol(&key, &const_binding_syms)
            } else {
                false
            };
