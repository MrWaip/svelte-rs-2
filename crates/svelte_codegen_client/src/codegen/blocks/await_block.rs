...
use svelte_analyze::{
    AwaitBinding, AwaitBlockSemantics, AwaitBranch, AwaitDestructureKind, AwaitWrapper, FragmentKey,
};
...
impl<'a, 'ctx> Codegen<'a, 'ctx> {
    ...
    fn build_await_pending_fn(
        ...
        let key = FragmentKey::AwaitPending(block_id);
    ...
    fn build_await_then_fn(
        ...
        if has_then {
            self.build_await_branch_callback(
                parent_ctx,
                FragmentKey::AwaitThen(block_id),
    ...
    fn build_await_catch_fn(
        ...
        if matches!(catch_branch, AwaitBranch::Present { .. }) {
            self.build_await_branch_callback(
                parent_ctx,
                FragmentKey::AwaitCatch(block_id),
    ...
    fn build_await_branch_callback(
        ...
        let mut inner_state = EmitState::new();
        self.emit_fragment(&mut inner_state, &inner_ctx, fragment_key)?;
