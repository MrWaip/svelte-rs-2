...
use oxc_span::SPAN;
use svelte_analyze::{FragmentKey, SnippetBlockSemantics, SnippetParam};
...
impl<'a, 'ctx> Codegen<'a, 'ctx> {
    ...
    fn build_snippet_const_inner(
        ...
        let Some(handle) = self.ctx.query.view.snippet_stmt_handle(block_id) else {
            return CodegenError::missing_expression(block_id);
        };
        let Some(parsed_stmt) = self.ctx.state.parsed.take_stmt(handle) else {
            return CodegenError::missing_expression(block_id);
        };
        ...
        let key = FragmentKey::SnippetBody(block_id);
        let mut inner_ctx = FragmentCtx::root(self.ctx, key);
