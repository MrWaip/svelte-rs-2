...
impl<'a, 'ctx> Codegen<'a, 'ctx> {
    ...
    fn build_let_directive_legacy_stmt(
        ...
        let handle = self.ctx.let_directive_stmt_handle(dir.id)?;
        ...
        let (is_destructured, simple_name, simple_init_clone): (
            ...
            let stmt_ref = self.ctx.state.parsed.stmt(handle)?;
        ...
        self.build_destructured_let_directive_legacy_stmt(handle)
    }

    fn build_destructured_let_directive_legacy_stmt(
        &mut self,
        handle: svelte_analyze::StmtHandle,
    ) -> Option<oxc_ast::ast::Statement<'a>> {
        use oxc_allocator::CloneIn;
