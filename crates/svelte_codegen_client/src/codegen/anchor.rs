...
impl<'a, 'ctx> Codegen<'a, 'ctx> {
        ...
        match &ctx.anchor {
            ...
            | FragmentAnchor::CallbackParam {
                ...
                let (frag_name, node_name) = match state.pending_anchor_idents.take() {
                    ...
                    Some((f, _)) => (
                        CompactString::from(f.as_str()),
                        CompactString::from(self.ctx.state.gen_ident("node")),
                    ),
                    None => (
                        CompactString::from(self.ctx.state.gen_ident("fragment")),
                        CompactString::from(self.ctx.state.gen_ident("node")),
                    ),
                };
                PreAnchor {
            ...
            FragmentAnchor::CallbackParam {
                ...
                let node_name = CompactString::from(self.ctx.state.gen_ident("node"));
                PreAnchor {
                    node_name,
                    frag_name: None,
    ...
    pub(in crate::codegen) fn commit_comment_anchor(
        ...
        if let Some(frag_name) = pre.frag_name {
            ...
            state.root_var = Some(frag_name.into());
            return Ok(node_name);
        }
