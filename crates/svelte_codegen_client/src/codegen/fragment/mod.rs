...
use super::data_structures::{FragmentAnchor, FragmentCtx};
use super::{Codegen, CodegenError, Result};

use prepare::{prepare, resolve_fragment, Child, ConcatPart, ContentStrategy, HoistedBucket};
...
impl StrategyKind {
    fn from(strategy: &ContentStrategy) -> Self {
        match strategy {
...
impl<'a, 'ctx> Codegen<'a, 'ctx> {
    pub(super) fn emit_fragment(
        ...
        let Some(fragment) = resolve_fragment(&key, self.ctx.query.component) else {
            return CodegenError::fragment_not_found(key);
        };

        let fragment_blockers = self.ctx.query.view.fragment_blockers(&key);
        ...
        let strategy_kind = StrategyKind::from(&strategy);
        ...
        self.emit_hoisted_const_tags(state, ctx, &bucket)?;
        ...
        use svelte_analyze::FragmentKeyExt;
        ...
        let emitted_prefix_next =
            key.needs_text_first_next() && starts_text_for_next && multi_or_root_callback;
        ...
        self.emit_hoisted_svelte_head_only(state, ctx, &bucket)?;
        self.emit_hoisted_special_targets(state, ctx, &bucket)?;
        if is_root_anchor {
            self.emit_hoisted_snippets_only(state, ctx, &bucket)?;
        }
        ...
        if is_root_anchor {
            self.emit_hoisted_titles_only(state, ctx, &bucket)?;
        } else {
            self.emit_hoisted_rest(state, ctx, &bucket)?;
        }
            ...
            && !state.suppress_root_finalize
        {
            ...
            self.finalize_root_template(state, ctx, strategy_kind, init_len_before, tpl_name, key)?;
        }
    ...
    fn build_template_locations(&self, key: FragmentKey) -> Option<oxc_ast::ast::Expression<'a>> {
        let fragment = resolve_fragment(&key, self.ctx.query.component)?;
    ...
    fn emit_component_with_css_wrapper(
        ...
        let css_props: Vec<(String, NodeId)> = self
            .ctx
            .query
            .view
    ...
    fn template_from_fn(
        ...
        if let StrategyKind::SingleElement = strategy_kind {
            if let Some(fragment) = super::prepare::resolve_fragment(&key, self.ctx.query.component)
            {
                for &id in &fragment.nodes {
                    if matches!(
                        self.ctx.query.component.store.get(id),
    ...
    pub(super) fn finalize_root_template(
        &mut self,
        state: &mut EmitState<'a>,
        ctx: &FragmentCtx<'a>,
        strategy_kind: StrategyKind,
        ...
        let from_fn = self.template_from_fn(ctx, key, strategy_kind);
        ...
        if self.ctx.state.dev {
            if let Some(locs) = self.build_template_locations(key) {
    ...
    fn emit_static_node(
        ...
        match &ctx.anchor {
            ...
            | FragmentAnchor::CallbackParam {
                ...
                let b = &self.ctx.state.b;
                if ctx.key.needs_text_first_next() {
                    state
                        .init
