...
pub(crate) fn gen_render_tag<'a>(
    ...
    for arg_src in &arg_sources {
        let thunk = build_thunked_arg(ctx, arg_src);
...
fn extract_call_parts(source: &str) -> (String, Vec<String>) {
    let paren_pos = match source.find('(') {
        Some(pos) => pos,
        None => return (source.trim().to_string(), vec![]),
    };

    let callee = source[..paren_pos].trim().to_string();
...
fn build_thunked_arg<'a>(ctx: &mut Ctx<'a>, source: &str) -> Expression<'a> {
    ...
    let (prop_sources, prop_non_sources) = {
        let mut ps = HashSet::new();
        let mut pns = HashMap::new();
        if let Some(pa) = &ctx.analysis.props {
            for p in &pa.props {
                if p.is_rest || p.is_prop_source {
                    ps.insert(p.local_name.clone());
                } else {
                    pns.insert(p.local_name.clone(), p.prop_name.clone());
                }
            }
        }
        (ps, pns)
