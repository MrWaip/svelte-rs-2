    ...
    match attr {
        ...
        Attribute::ExpressionAttribute(a) => {
            if let Some(raw_event_name) = a.event_name.as_deref() {
                ...
                let event_name = svelte_types::strip_capture_event(raw_event_name)
...
pub(crate) fn gen_event_attr_on<'a>(
    ...
    let (event_name, capture) = if let Some(base) = svelte_types::strip_capture_event(raw_event_name) {
        (base.to_string(), true)
    } else {
    ...
    let passive = svelte_types::is_passive_event(&event_name);
    let mut args: Vec<Arg<'a, '_>> = vec![
        Arg::Str(event_name.clone()),
        Arg::Ident(target),
        Arg::Expr(handler),
