...
export default function App($$anchor, $$props) {
	...
	$.add_svelte_meta(() => $.each(node, 1, $list, $.index, ($$anchor, item, $$index) => {
		var button = root();
		var text = $.child(button, true);
		$.reset(button);
		$.template_effect(() => $.set_text(text, $list()[$$index]));
		$.event("click", button, function click() {
			return $list()[$$index] = $list()[$$index] + 1, $.invalidate_inner_signals(() => $list()), $.invalidate_store($$stores, "$list");
		});
