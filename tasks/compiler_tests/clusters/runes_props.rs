use super::*;

compiler_case!(
    assignment_array,
    "runes/props/assignment/array",
    [prod, dev, ssr, ssr_dev]
);
compiler_case!(
    assignment_array_rest,
    "runes/props/assignment/array_rest",
    [prod, dev, ssr, ssr_dev]
);
compiler_case!(
    assignment_computed_key,
    "runes/props/assignment/computed_key",
    [prod, dev, ssr, ssr_dev]
);
compiler_case!(
    assignment_default,
    "runes/props/assignment/default",
    [prod, dev, ssr, ssr_dev]
);
compiler_case!(
    assignment_nested_array,
    "runes/props/assignment/nested_array",
    [prod, dev, ssr, ssr_dev]
);
compiler_case!(
    assignment_object,
    "runes/props/assignment/object",
    [prod, dev, ssr, ssr_dev]
);
compiler_case!(
    assignment_object_rest,
    "runes/props/assignment/object_rest",
    [prod, dev, ssr, ssr_dev]
);
compiler_case!(
    declaration_alias,
    "runes/props/declaration/alias",
    [prod, dev, ssr, ssr_dev]
);
compiler_case!(
    declaration_bindable,
    "runes/props/declaration/bindable",
    [prod, dev, ssr, ssr_dev]
);
compiler_case!(
    declaration_bindable_alias,
    "runes/props/declaration/bindable_alias",
    [prod, dev, ssr, ssr_dev]
);
compiler_case!(
    declaration_bindable_default,
    "runes/props/declaration/bindable_default",
    [prod, dev, ssr, ssr_dev]
);
compiler_case!(
    declaration_default_alias,
    "runes/props/declaration/default_alias",
    [prod, dev, ssr, ssr_dev]
);
compiler_case!(
    declaration_default_leaf,
    "runes/props/declaration/default_leaf",
    [prod, dev, ssr, ssr_dev]
);
compiler_case!(
    declaration_flat_object,
    "runes/props/declaration/flat_object",
    [prod, dev, ssr, ssr_dev]
);
compiler_case!(
    declaration_mixed_rest_default,
    "runes/props/declaration/mixed_rest_default",
    [prod, dev, ssr, ssr_dev]
);
compiler_case!(
    declaration_rest,
    "runes/props/declaration/rest",
    [prod, dev, ssr, ssr_dev]
);
compiler_case!(
    declaration_single,
    "runes/props/declaration/single",
    [prod, dev, ssr, ssr_dev]
);
compiler_case!(
    declaration_string_key,
    "runes/props/declaration/string_key",
    [prod, dev, ssr, ssr_dev]
);
compiler_case!(
    optional_member_derived,
    "runes/props/optional_member_derived",
    [prod, dev, ssr, ssr_dev]
);
compiler_case!(
    optional_computed_member_guard,
    "runes/props/optional_computed_member_guard",
    [prod, dev, ssr, ssr_dev]
);
compiler_case!(
    static_member_derived_guard,
    "runes/props/static_member_derived_guard",
    [prod, dev, ssr, ssr_dev]
);
