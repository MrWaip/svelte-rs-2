use std::{
    fs::{File, read_to_string},
    io::Write,
};

use compiler_tests::cases::{load_v3_case, load_v3_module_case, v3_case_dir};
use compiler_tests::sourcemap_invariants::assert_sourcemap_invariants;
use compiler_tests::{compiler_case, compiler_module_case};
use pretty_assertions::assert_eq;
use svelte_compiler::{GenerateMode, compile, compile_module};
use test_support::strip_reference_only_css_markers;

fn normalize_css(s: &str) -> String {
    let stripped = strip_reference_only_css_markers(s);
    stripped.split_whitespace().collect::<Vec<_>>().join(" ")
}

fn assert_compiler_prod(case: &str) {
    let dir = v3_case_dir(case);
    let (input, opts) = load_v3_case(case);
    let result = compile(&input, &opts);
    let js_output = result
        .js
        .unwrap_or_else(|| panic!("[{case}] compile produced no JS"));
    let js = js_output.code;

    let expected_js = read_to_string(dir.join("case-svelte.js")).expect("test invariant");

    File::create(dir.join("case-rust.js"))
        .expect("test invariant")
        .write_all(js.as_bytes())
        .expect("test invariant");

    assert_eq!(js, expected_js, "[{case}] JS mismatch");

    if let Some(map) = js_output.map.as_ref() {
        assert_sourcemap_invariants(case, &input, map, svelte_compiler::SourcemapKind::Default);
    }

    let expected_css_path = dir.join("case-svelte.css");
    if expected_css_path.exists() {
        let expected_css = read_to_string(&expected_css_path).expect("test invariant");
        let actual_css = result.css.map(|out| out.code).unwrap_or_default();
        File::create(dir.join("case-rust.css"))
            .expect("test invariant")
            .write_all(actual_css.as_bytes())
            .expect("test invariant");
        assert_eq!(
            normalize_css(&actual_css),
            normalize_css(&expected_css),
            "[{case}] CSS mismatch"
        );
    }
}

fn assert_compiler_dev(case: &str) {
    let dir = v3_case_dir(case);
    let (input, opts) = load_v3_case(case);
    let mut dev_opts = opts.clone();
    dev_opts.dev = true;
    let dev_js = compile(&input, &dev_opts)
        .js
        .unwrap_or_else(|| panic!("[{case}] dev compile produced no JS"))
        .code;
    let expected_dev_js = read_to_string(dir.join("case-svelte.dev.js")).expect("test invariant");
    File::create(dir.join("case-rust.dev.js"))
        .expect("test invariant")
        .write_all(dev_js.as_bytes())
        .expect("test invariant");
    assert_eq!(dev_js, expected_dev_js, "[{case}] dev JS mismatch");
}

fn assert_compiler_ssr(case: &str) {
    let dir = v3_case_dir(case);
    let (input, opts) = load_v3_case(case);
    let mut server_opts = opts.clone();
    server_opts.generate = GenerateMode::Server;
    let server_js = compile(&input, &server_opts)
        .js
        .unwrap_or_else(|| panic!("[{case}] server compile produced no JS"))
        .code;
    let expected_server_js =
        read_to_string(dir.join("case-svelte.server.js")).expect("test invariant");
    File::create(dir.join("case-rust.server.js"))
        .expect("test invariant")
        .write_all(server_js.as_bytes())
        .expect("test invariant");
    assert_eq!(server_js, expected_server_js, "[{case}] server JS mismatch");
}

fn assert_compiler_ssr_dev(case: &str) {
    let dir = v3_case_dir(case);
    let (input, opts) = load_v3_case(case);
    let mut server_opts = opts.clone();
    server_opts.generate = GenerateMode::Server;
    server_opts.dev = true;
    let server_js = compile(&input, &server_opts)
        .js
        .unwrap_or_else(|| panic!("[{case}] server dev compile produced no JS"))
        .code;
    let expected_server_js =
        read_to_string(dir.join("case-svelte.server.dev.js")).expect("test invariant");
    File::create(dir.join("case-rust.server.dev.js"))
        .expect("test invariant")
        .write_all(server_js.as_bytes())
        .expect("test invariant");
    assert_eq!(
        server_js, expected_server_js,
        "[{case}] server dev JS mismatch"
    );
}

compiler_case!(
    legacy_const_each_bind_member_chain,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_js_object_method_shorthand,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_props_default_identifier_prop_reference,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_props_default_identifier_non_reactive,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(css_scope_class_in_snippet, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_scope_svelte_element_class, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_scope_class_object, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    css_scope_class_array_no_directive,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(css_scope_class_array_with_state, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_scope_spread_attribute, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_unused_external, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_unused_injected, [prod, dev_todo, ssr, ssr_dev_todo]);

compiler_case!(css_nested_style, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    css_nested_pseudo_element_no_scope_class,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    css_nested_amp_compound_no_scope_class,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_css_nested_sibling_amp_scope_class,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_css_animation_vendor_prefix,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_css_vendor_keyframes_rename,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(css_scoped_id_selector, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_scoped_attr_presence, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_scoped_attr_value_selector, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_scoped_attr_matcher_operators, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_scoped_attr_name_casefolding, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    css_pseudo_compound_unused_but_scoped,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    css_scope_child_combinator_bare_pseudo,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    css_scope_child_combinator_global_pseudo_unscoped,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    css_snippet_descendant_scope_boundary,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(css_snippet_sibling_boundary, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_css_recursive_snippet_sibling_overflow,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    css_component_snippet_descendant_boundary,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(css_pseudo_has, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_pseudo_not_scoped, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_nesting_selector_scoped, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_root_has_scoped, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_escaped_selector_scoped, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_dynamic_attr_selector_match, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_comments_preserved, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    script_module_exports_ordering_with_snippets,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(script_jsdoc_preserve, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_raw_dev_ce_with_props_rest, [prod, dev, ssr, ssr_dev]);

compiler_case!(warn_attr_avoid_is, [prod, dev, ssr, ssr_dev]);

compiler_case!(warn_attr_illegal_colon, [prod, dev, ssr, ssr_dev]);

compiler_case!(warn_attr_invalid_prop_name, [prod, dev, ssr, ssr_dev]);

compiler_case!(warn_slot_deprecated, [prod, dev, ssr, ssr_dev]);

compiler_case!(slot_named_fallback, [prod, dev, ssr, ssr_dev]);

compiler_case!(legacy_slot_dev_mixed, [prod, dev, ssr, ssr_dev]);

compiler_case!(slot_props_default, [prod, dev, ssr, ssr_dev]);

compiler_case!(slot_props_spread, [prod, dev, ssr, ssr_dev]);

compiler_case!(slot_props_dynamic_state, [prod, dev, ssr, ssr_dev]);

compiler_case!(slot_props_dynamic_call, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_legacy_slot_props_store_member,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_slot_prop_conditional_no_derived_wrap,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_slot_prop_non_simple_stateful_shapes,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(warn_script_context_deprecated, [prod, dev, ssr, ssr_dev]);

compiler_case!(head_with_special_elements, [prod, dev, ssr, ssr_dev]);

compiler_case!(head_with_snippets, [prod, dev, ssr, ssr_dev]);

compiler_case!(head_with_if_body, [prod, dev, ssr, ssr_dev]);

compiler_case!(head_with_render_and_component, [prod, dev, ssr, ssr_dev]);

compiler_case!(diagnose_head_script_in_if, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_head_inline_script_template_literal,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    head_nested_if_with_body_if_id_order,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(head_title_then_meta_effect_order, [prod, dev, ssr, ssr_dev]);

compiler_case!(push_binding_group_order, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_group_order_with_stores, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    bind_group_order_with_legacy_reactive,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    component_bind_group_multiple_targets,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(bind_group_value_defined, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_member_expression_no_runes, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    legacy_const_destructured_member_bind,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_const_destructure_keeps_siblings,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_const_member_mutation_through_ts_non_null,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    css_injected_append_styles_with_stores_order,
    [prod, dev_todo, ssr, ssr_dev_todo]
);

compiler_case!(css_scoped_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_injected, [prod, dev_todo, ssr, ssr_dev_todo]);

compiler_case!(css_global_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_global_block, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_global_compound, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_global_in_pseudo, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_global_with_combinators, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_css_global_leading_combinator,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(css_keyframes_scoped, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    css_keyframes_percentage_scopes_all,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    bind_this_with_children_and_class_directive,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(head_position_with_body, [prod, dev, ssr, ssr_dev]);

compiler_case!(special_elements_all, [prod, dev, ssr, ssr_dev]);

compiler_case!(empty, [prod, dev, ssr, ssr_dev]);

compiler_case!(simple, "hello_state", [prod, dev, ssr, ssr_dev]);

compiler_case!(single_text_node, [prod, dev, ssr, ssr_dev]);

compiler_case!(single_element, [prod, dev, ssr, ssr_dev]);

compiler_case!(single_interpolation, [prod, dev, ssr, ssr_dev]);

compiler_case!(text_entity_decoding, [prod, dev, ssr, ssr_dev]);

compiler_case!(text_entity_decoding_root, [prod, dev, ssr, ssr_dev]);

compiler_case!(title_entity_decoding, [prod, dev, ssr, ssr_dev]);

compiler_case!(single_if_block, [prod, dev, ssr, ssr_dev]);

compiler_case!(single_if_else_block, [prod, dev, ssr, ssr_dev]);

compiler_case!(if_call_condition, [prod, dev, ssr, ssr_dev]);

compiler_case!(if_block_empty_consequent, [prod, dev, ssr, ssr_dev]);

compiler_case!(if_block_empty_alternate, [prod, dev, ssr, ssr_dev]);

compiler_case!(element_attributes, [prod, dev, ssr, ssr_dev]);

compiler_case!(element_autofocus, [prod, dev, ssr, ssr_dev]);

compiler_case!(textarea_child_value_dynamic, [prod, dev, ssr, ssr_dev]);

compiler_case!(option_expr_child_value, [prod, dev, ssr, ssr_dev]);

compiler_case!(option_expr_value, [prod, dev, ssr, ssr_dev]);

compiler_case!(option_concat_value, [prod, dev, ssr, ssr_dev]);

compiler_case!(option_expr_value_multi, [prod, dev, ssr, ssr_dev]);

compiler_case!(option_expr_value_defined, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    option_expr_value_use_action_cache_var_order,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(bind_value_dev_named_fns, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_component_prop_dev_ownership, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    bind_component_plain_prop_dev_ownership,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    bind_dynamic_component_dev_ownership,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    bind_component_dev_ownership_ignore,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(bind_component_explicit_source, [prod, dev, ssr, ssr_dev]);

compiler_case!(customizable_select_option_el, [prod, dev, ssr, ssr_dev]);

compiler_case!(customizable_select_select_div, [prod, dev, ssr, ssr_dev]);

compiler_case!(selectedcontent_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_runes, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_raw, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_eager_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_eager_reactive, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_eager_template, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_snapshot_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_snapshot_expression, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_snapshot_reactive, [prod, dev, ssr, ssr_dev]);

compiler_case!(each_block, [prod, dev, ssr, ssr_dev]);

compiler_case!(each_inner_shadow, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    each_nested_array_destructure_no_inner_shadow,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    each_legacy_shadow_with_script_array_assign,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(bind_directives, [prod, dev, ssr, ssr_dev]);

compiler_case!(nested_elements, [prod, dev, ssr, ssr_dev]);

compiler_case!(nested_resets, [prod, dev, ssr, ssr_dev]);

compiler_case!(single_concatenation, [prod, dev, ssr, ssr_dev]);

compiler_case!(elements_childs, [prod, dev, ssr, ssr_dev]);

compiler_case!(generic_root_sequence, [prod, dev, ssr, ssr_dev]);

compiler_case!(spread_attribute, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    attribute_effect_shorthand_prop_unwrap,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(img_spread_replay_events, [prod, dev, ssr, ssr_dev]);

compiler_case!(embed_spread_replay_events, [prod, dev, ssr, ssr_dev]);

compiler_case!(object_spread_replay_events, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    source_spread_in_each_no_replay_events,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(spread_class_directive, [prod, dev, ssr, ssr_dev]);

compiler_case!(spread_style_directive, [prod, dev, ssr, ssr_dev]);

compiler_case!(utf8, [prod, dev, ssr, ssr_dev]);

compiler_case!(smoke, [prod, dev, ssr, ssr_dev]);

compiler_case!(class_directive, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_class_directive_name_with_underscore,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_class_directive_named_like_slot_attribute_no_placement_error,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_class_directive_call_in_template_effect,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_class_directive_slots_member_legacy,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_attribute_effect_spread_call_memo,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_attribute_effect_concat_call_part_memo,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_attribute_effect_hydration_ignored_eighth_arg,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_set_attribute_hydration_ignored_fourth_arg,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_set_xlink_attribute_hydration_ignored_fourth_arg,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(class_concat, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    empty_class_attribute_static_elided,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_class_directive_on_svg_is_html_flag,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(class_concat_literal_fold, [prod, dev, ssr, ssr_dev]);

compiler_case!(attribute_concat_literal_fold, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    component_prop_concat_literal_fold,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(rune_update, [prod, dev, ssr, ssr_dev]);

compiler_case!(assign_in_template, [prod, dev, ssr, ssr_dev]);

compiler_case!(only_script, [prod, dev, ssr, ssr_dev]);

compiler_case!(hoist_imports, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_directives_extended, [prod, dev, ssr, ssr_dev]);

compiler_case!(mutated_state_rune, [prod, dev, ssr, ssr_dev]);

compiler_case!(static_interpolation, [prod, dev, ssr, ssr_dev]);

compiler_case!(props_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(props_rest, [prod, dev, ssr, ssr_dev]);

compiler_case!(props_renamed, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    props_const_destructured_with_default,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    template_effect_merge_class_state_with_memo_text,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(props_renamed_bindable, [prod, dev, ssr, ssr_dev]);

compiler_case!(props_bindable, [prod, dev, ssr, ssr_dev]);

compiler_case!(props_lazy_default, [prod, dev, ssr, ssr_dev]);

compiler_case!(props_mutated, [prod, dev, ssr, ssr_dev]);

compiler_case!(props_member_mutation_computed, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    props_renamed_member_update_computed,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(props_mixed, [prod, dev, ssr, ssr_dev]);

compiler_case!(exports, [prod, dev, ssr, ssr_dev]);

compiler_case!(snippet_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_component_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_component_children, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_component_if_child, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_component_each_child, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    svelte_component_slot_legacy_store_reactivity,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    svelte_component_slot_legacy_if_store_untrack,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(component_non_self_closing, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_in_element, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_mixed, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_props, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_children, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_component_slot_node_naming,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_sibling_after_deep_nested_elements,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_slot_fallback_if_sibling_node_naming,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(component_events, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_events_dev_apply, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_element_children, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_named_slot, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    legacy_slot_forward_named_into_child_component,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(component_default_slot_let, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_component_callee_from_slot_let,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_callee_member_legacy_reactive_root,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_callee_from_slot_let_shadowed_by_script_binding,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    component_let_directive_name_starts_with_on,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_events_slot_let_handler,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_attr_on_prefix_false_positive,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(component_default_slot_let_alias, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_named_slot_let_element, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    component_named_slot_let_element_destructure,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    component_named_slot_let_element_multiple,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(component_child_slot_attribute, [prod, dev, ssr, ssr_dev]);

compiler_case!(smoke_all, [prod, dev, ssr, ssr_dev]);

compiler_case!(derived_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_derived_rune_with_ts_as_cast,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(derived_by, [prod, dev, ssr, ssr_dev]);

compiler_case!(derived_dynamic, [prod, dev, ssr, ssr_dev]);

compiler_case!(derived_write_assignment, [prod, dev, ssr, ssr_dev]);

compiler_case!(unmutated_state_optimization, [prod, dev, ssr, ssr_dev]);

compiler_case!(effect_runes, [prod, dev, ssr, ssr_dev]);

compiler_case!(effect_root_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(effect_root_cleanup, [prod, dev, ssr, ssr_dev]);

compiler_case!(effect_tracking, [prod, dev, ssr, ssr_dev]);

compiler_case!(effect_pending, [prod, dev, ssr, ssr_dev]);

compiler_case!(effect_pending_script_init, [prod, dev, ssr, ssr_dev]);

compiler_case!(effect_pending_script_derived, [prod, dev, ssr, ssr_dev]);

compiler_case!(host_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(host_props_rest, [prod, dev, ssr, ssr_dev]);

compiler_case!(custom_element_props, [prod, dev, ssr, ssr_dev]);

compiler_case!(custom_element_props_config, [prod, dev, ssr, ssr_dev]);

compiler_case!(custom_element_boolean_default, [prod, dev, ssr, ssr_dev]);

compiler_case!(custom_element_exports, [prod, dev, ssr, ssr_dev]);

compiler_case!(custom_element_shadow_none, [prod, dev, ssr, ssr_dev]);

compiler_case!(custom_element_object_full, [prod, dev, ssr, ssr_dev]);

compiler_case!(custom_element_shadow_open, [prod, dev, ssr, ssr_dev]);

compiler_case!(custom_element_extend, [prod, dev, ssr, ssr_dev]);

compiler_case!(custom_element_no_tag, [prod, dev, ssr, ssr_dev]);

compiler_case!(custom_element_prop_alias, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    custom_element_compile_option_default,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    custom_element_dev_exports_legacy_api,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(custom_element_slots, [prod, dev, ssr, ssr_dev]);

compiler_case!(legacy_props_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(legacy_reactivity_let_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(legacy_reactivity_var_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(legacy_reactivity_member_mutation, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    legacy_reactivity_array_self_assign,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(legacy_reactivity_destructure, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_legacy_array_destructure_mixed_targets,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(legacy_reactive_assignment_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_legacy_pre_effect_reset_with_module_script,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_reactive_assignment_declared_dependency,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_reactive_assignment_block_destructure,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_reactive_assignment_coarse_deps,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_reactive_assignment_import_topology,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(legacy_rest_props_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(legacy_slots_if, [prod, dev, ssr, ssr_dev]);

compiler_case!(legacy_slots_script_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(legacy_before_after_update_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(legacy_before_after_update_alias, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    custom_element_css_default_injected,
    [prod, dev_todo, ssr, ssr_dev]
);

compiler_case!(custom_element_shadow_object, [prod, dev, ssr, ssr_dev]);

compiler_case!(html_tag, [prod, dev, ssr, ssr_dev]);

compiler_case!(html_tag_mathml, [prod, dev, ssr, ssr_dev]);

compiler_case!(svg_foreignobject_fragment_html, [prod, dev, ssr, ssr_dev]);

compiler_case!(mathml_root_html_fragment, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    mathml_annotation_xml_fragment_html,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(key_block, [prod, dev, ssr, ssr_dev]);

compiler_case!(key_block_nested, [prod, dev, ssr, ssr_dev]);

compiler_case!(style_directive, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_custom_prop_component, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_css_prop_boolean, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_custom_prop_component_svg, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_custom_prop_component_nested, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    css_custom_prop_component_string_value,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    css_custom_prop_component_concat_value,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    css_custom_prop_component_with_memoized_prop,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    css_custom_prop_component_slot_fill,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(style_directive_important, [prod, dev, ssr, ssr_dev]);

compiler_case!(style_directive_string, [prod, dev, ssr, ssr_dev]);

compiler_case!(style_directive_concat, [prod, dev, ssr, ssr_dev]);

compiler_case!(on_directive, [prod, dev, ssr, ssr_dev]);

compiler_case!(on_directive_modifiers, [prod, dev, ssr, ssr_dev]);

compiler_case!(on_directive_with_use, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    on_directive_with_use_bind_this_order,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(on_directive_nonpassive, [prod, dev, ssr, ssr_dev]);

compiler_case!(on_directive_dev_apply, [prod, dev, ssr, ssr_dev]);

compiler_case!(use_action_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(use_action_expression, [prod, dev, ssr, ssr_dev]);

compiler_case!(use_action_reactive, [prod, dev, ssr, ssr_dev]);

compiler_case!(use_action_dotted, [prod, dev, ssr, ssr_dev]);

compiler_case!(use_action_dotted_hyphen, [prod, dev, ssr, ssr_dev]);

compiler_case!(use_action_multiple, [prod, dev, ssr, ssr_dev]);

compiler_case!(use_action_in_if, [prod, dev, ssr, ssr_dev]);

compiler_case!(use_action_in_each, [prod, dev, ssr, ssr_dev]);

compiler_case!(use_action_with_children, [prod, dev, ssr, ssr_dev]);

compiler_case!(void_elements, [prod, dev, ssr, ssr_dev]);

compiler_case!(non_void_self_closing, [prod, dev, ssr, ssr_dev]);

compiler_case!(mixed_html_elements, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    store_legacy_let_synthetic_reassign,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(store_legacy_each_invalidate, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_legacy_each_member_iterable, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_bind_value_thunk_arrow, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_legacy_var_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_legacy_member_mutation, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_runes_id_assign_ops, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_runes_id_ops_template, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_runes_member_ops_script, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_runes_member_ops_template, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_runes_computed_member, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_runes_dev_smoke, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_runes_each_member_mutation, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_runes_prop_thunk, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    store_runes_synthetic_thunk_derived_base,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(store_runes_prop_assign_bind, [prod, dev, ssr, ssr_dev]);

compiler_case!(diagnose_bindable_prop_store_only, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    store_runes_component_bind_prop_store,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(component_bind_ref_state_flag, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_bind_prop_order, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    component_snippet_node_ident_ordering,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(store_legacy_id_assign_ops, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_legacy_id_ops_template, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_legacy_member_ops_script, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_legacy_dev_smoke, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_legacy_bind_value, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_legacy_bind_value_writable_store_shadow,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(legacy_dev_synthetic_store_thunk, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    legacy_dev_writable_no_mutable_source,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(legacy_dev_store_thunk_call_read, [prod, dev, ssr, ssr_dev]);

compiler_case!(legacy_dev_store_set_assignment, [prod, dev, ssr, ssr_dev]);

compiler_case!(legacy_dev_bind_store_unsub, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    legacy_dev_bind_this_promotes_state,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_dev_reactive_text_only_element,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_dev_deferred_template_effect,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_head_title_store_deps_coarse_wrap,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(store_if_block_condition, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_key_block_expression, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_await_block_promise, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_render_tag_snippet, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_bind_this_element_ref, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_bare_identifier_method_call, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_write, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_validate_dev, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_reassign_unsub, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_each_invalidate, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_mark_binding, [prod, dev, ssr, ssr_dev]);

compiler_case!(const_tag, [prod, dev, ssr, ssr_dev]);

compiler_case!(const_tag_destructured, [prod, dev, ssr, ssr_dev]);

compiler_case!(const_tag_destructured_multi, [prod, dev, ssr, ssr_dev]);

compiler_case!(const_tag_destructured_if, [prod, dev, ssr, ssr_dev]);

compiler_case!(const_tag_destructured_single_key, [prod, dev, ssr, ssr_dev]);

compiler_case!(const_tag_destructured_default, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_const_tag_legacy_destructure_into_component,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(const_tag_key_block, [prod, dev, ssr, ssr_dev]);

compiler_case!(const_tag_await, [prod, dev, ssr, ssr_dev]);

compiler_case!(const_tag_component, [prod, dev, ssr, ssr_dev]);

compiler_case!(class_array, [prod, dev, ssr, ssr_dev]);

compiler_case!(class_object, [prod, dev, ssr, ssr_dev]);

compiler_case!(class_variable, [prod, dev, ssr, ssr_dev]);

compiler_case!(class_expr_with_directives, [prod, dev, ssr, ssr_dev]);

compiler_case!(class_concat_logical_and_string, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_select_value, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_select_value_attribute_emits_special_value_triple,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(bind_files, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_property, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_content_editable, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_element_size, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    bind_element_size_bindable_prop_source,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(bind_resize_observer, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    bind_resize_observer_border_box_size,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    bind_resize_observer_device_pixel_content_box_size,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(bind_textarea_value, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_media_rw, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_media_ro, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_media_property, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_img, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_this, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_bind_this, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_bind_this_variants, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_self_if, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_self_if_sibling, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_self_if_else_sibling, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_self_each_sibling, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_self_element_sibling, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_self_css_props, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_self_each, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_self_snippet, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_self_slot, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_self_props, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_self_bind_this, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_focused, [prod, dev, ssr, ssr_dev]);

// ---------------------------------------------------------------------------
// Transition tests
// ---------------------------------------------------------------------------

compiler_case!(transition_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(transition_params, [prod, dev, ssr, ssr_dev]);

compiler_case!(transition_in, [prod, dev, ssr, ssr_dev]);

compiler_case!(transition_out, [prod, dev, ssr, ssr_dev]);

compiler_case!(transition_in_out_separate, [prod, dev, ssr, ssr_dev]);

compiler_case!(transition_local, [prod, dev, ssr, ssr_dev]);

compiler_case!(transition_global, [prod, dev, ssr, ssr_dev]);

compiler_case!(transition_dotted_name, [prod, dev, ssr, ssr_dev]);

compiler_case!(transition_in_if, [prod, dev, ssr, ssr_dev]);

compiler_case!(transition_reactive_params, [prod, dev, ssr, ssr_dev]);

compiler_case!(transition_elseif_local, [prod, dev, ssr, ssr_dev]);

compiler_case!(transition_after_delegated, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    transition_after_delegated_descendant,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    transition_before_lifecycle_events,
    [prod, dev, ssr, ssr_dev]
);

// ---------------------------------------------------------------------------
// Animate directive tests
// ---------------------------------------------------------------------------

compiler_case!(animate_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(animate_params, [prod, dev, ssr, ssr_dev]);

compiler_case!(animate_dotted_name, [prod, dev, ssr, ssr_dev]);

compiler_case!(animate_reactive_params, [prod, dev, ssr, ssr_dev]);

compiler_case!(animate_svelte_element, [prod, dev, ssr, ssr_dev]);

compiler_case!(animate_with_const_tag, [prod, dev, ssr, ssr_dev]);

// ---------------------------------------------------------------------------
// Attach tag tests
// ---------------------------------------------------------------------------

compiler_case!(attach_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(attach_inline_arrow, [prod, dev, ssr, ssr_dev]);

compiler_case!(attach_conditional, [prod, dev, ssr, ssr_dev]);

compiler_case!(attach_multiple, [prod, dev, ssr, ssr_dev]);

compiler_case!(attach_with_directives, [prod, dev, ssr, ssr_dev]);

compiler_case!(attach_in_if, [prod, dev, ssr, ssr_dev]);

compiler_case!(attach_in_each, [prod, dev, ssr, ssr_dev]);

// ---------------------------------------------------------------------------
// $state/$state.raw destructuring
// ---------------------------------------------------------------------------

compiler_case!(state_destructure, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_raw_destructure_object, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_raw_destructure_array, [prod, dev, ssr, ssr_dev]);

// ---------------------------------------------------------------------------
// $state/$state.raw class fields
// ---------------------------------------------------------------------------

compiler_case!(state_class_field, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_raw_class_field, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_private_class_field, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_class_constructor, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_class_multiple, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_constructor_private_read, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_constructor_read_v, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_constructor_read_derived, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_class_raw_field, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_no_init, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_snapshot_in_template, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_snapshot_ignored, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_snapshot_not_ignored, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_snapshot_ignored_return, [prod, dev, ssr, ssr_dev]);

compiler_case!(for_await_ignored, [prod, dev, ssr, ssr_dev]);

compiler_case!(await_reactivity_ignored, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_class_constructor_proxy, [prod, dev, ssr, ssr_dev]);

compiler_case!(derived_class_field, [prod, dev, ssr, ssr_dev]);

compiler_case!(derived_by_class_fields, [prod, dev, ssr, ssr_dev]);

compiler_case!(derived_by_class_constructor_only, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    derived_by_class_placeholder_preserves_plain_fields,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    state_class_field_constructor_assign,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(svg_inner_whitespace_trimming, [prod, dev, ssr, ssr_dev]);

compiler_case!(svg_inner_template_from_svg, [prod, dev, ssr, ssr_dev]);

compiler_case!(template_effect_call_deps, [prod, dev, ssr, ssr_dev]);

compiler_case!(svg_text_preserves_whitespace, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    template_effect_multiple_call_deps,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    template_effect_attr_before_text_order,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    component_local_underscored_bind_this,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    component_dynamic_dotted_identifier_root,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(component_dynamic_props_access, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    component_dynamic_dotted_props_root,
    [prod, dev, ssr, ssr_dev]
);

// ---------------------------------------------------------------------------
// Module compilation tests
// ---------------------------------------------------------------------------

fn strip_leading_block_comments(src: &str) -> String {
    let bytes = src.as_bytes();
    let mut out = String::with_capacity(src.len());
    let mut i = 0;
    while i < bytes.len() {
        if i + 2 < bytes.len() && bytes[i] == b'/' && bytes[i + 1] == b'*' && bytes[i + 2] == b'*' {
            let mut j = i + 3;
            while j + 1 < bytes.len() && !(bytes[j] == b'*' && bytes[j + 1] == b'/') {
                j += 1;
            }
            if j + 1 >= bytes.len() {
                out.push_str(&src[i..]);
                break;
            }
            let mut k = j + 2;
            while k < bytes.len() && matches!(bytes[k], b' ' | b'\t') {
                k += 1;
            }
            if k < bytes.len() && bytes[k] == b'\n' {
                k += 1;
                let line_start = out.rfind('\n').map_or(0, |p| p + 1);
                if out[line_start..].chars().all(|c| c == ' ' || c == '\t') {
                    out.truncate(line_start);
                }
                i = k;
            } else {
                i = j + 2;
            }
            continue;
        }
        out.push(bytes[i] as char);
        i += 1;
    }
    out
}

fn assert_compiler_module_prod(case: &str) {
    let dir = v3_case_dir(case);
    let (input, opts) = load_v3_module_case(case);

    let result = compile_module(&input, &opts);
    let js_output = result
        .js
        .unwrap_or_else(|| panic!("[{case}] compile_module produced no JS"));
    let js = js_output.code;

    let expected = read_to_string(dir.join("case-svelte.js")).expect("test invariant");

    File::create(dir.join("case-rust.js"))
        .expect("test invariant")
        .write_all(js.as_bytes())
        .expect("test invariant");
    assert_eq!(
        strip_leading_block_comments(&js),
        strip_leading_block_comments(&expected)
    );

    if let Some(map) = js_output.map.as_ref() {
        assert_sourcemap_invariants(case, &input, map, svelte_compiler::SourcemapKind::Default);
    }
}

fn assert_compiler_module_dev(case: &str) {
    let dir = v3_case_dir(case);
    let (input, opts) = load_v3_module_case(case);

    let mut dev_opts = opts.clone();
    dev_opts.dev = true;
    let dev_js = compile_module(&input, &dev_opts)
        .js
        .unwrap_or_else(|| panic!("[{case}] dev compile_module produced no JS"))
        .code;
    let expected_dev = read_to_string(dir.join("case-svelte.dev.js")).expect("test invariant");
    File::create(dir.join("case-rust.dev.js"))
        .expect("test invariant")
        .write_all(dev_js.as_bytes())
        .expect("test invariant");
    assert_eq!(
        strip_leading_block_comments(&dev_js),
        strip_leading_block_comments(&expected_dev),
        "[{case}] dev JS mismatch"
    );
}

fn assert_compiler_module_ssr(case: &str) {
    let dir = v3_case_dir(case);
    let (input, opts) = load_v3_module_case(case);

    let mut server_opts = opts.clone();
    server_opts.generate = GenerateMode::Server;
    let server_js = compile_module(&input, &server_opts)
        .js
        .unwrap_or_else(|| panic!("[{case}] server compile_module produced no JS"))
        .code;
    let expected_server =
        read_to_string(dir.join("case-svelte.server.js")).expect("test invariant");
    File::create(dir.join("case-rust.server.js"))
        .expect("test invariant")
        .write_all(server_js.as_bytes())
        .expect("test invariant");
    assert_eq!(
        strip_leading_block_comments(&server_js),
        strip_leading_block_comments(&expected_server),
        "[{case}] server JS mismatch"
    );
}

fn assert_compiler_module_ssr_dev(case: &str) {
    let dir = v3_case_dir(case);
    let (input, opts) = load_v3_module_case(case);

    let mut server_dev_opts = opts.clone();
    server_dev_opts.generate = GenerateMode::Server;
    server_dev_opts.dev = true;
    let server_dev_js = compile_module(&input, &server_dev_opts)
        .js
        .unwrap_or_else(|| panic!("[{case}] server dev compile_module produced no JS"))
        .code;
    let expected_server_dev =
        read_to_string(dir.join("case-svelte.server.dev.js")).expect("test invariant");
    File::create(dir.join("case-rust.server.dev.js"))
        .expect("test invariant")
        .write_all(server_dev_js.as_bytes())
        .expect("test invariant");
    assert_eq!(
        strip_leading_block_comments(&server_dev_js),
        strip_leading_block_comments(&expected_server_dev),
        "[{case}] server dev JS mismatch"
    );
}

compiler_module_case!(
    state_class_field_proxy_init,
    "state_class_field_proxy_init",
    [prod, dev, ssr, ssr_dev]
);

compiler_module_case!(
    module_derived_arrow_wrap_with_state_deps,
    "module_derived_arrow_wrap_with_state_deps",
    [prod, dev, ssr, ssr_dev]
);

compiler_module_case!(
    module_state_destructure,
    "module_state_destructure",
    [prod, dev, ssr, ssr_dev]
);

compiler_module_case!(
    module_compilation,
    "module_compilation",
    [prod, dev, ssr, ssr_dev]
);

compiler_module_case!(
    state_raw_class_constructor_object_ts,
    "state_raw_class_constructor_object_ts",
    [prod, dev, ssr, ssr_dev]
);

compiler_module_case!(
    module_derived_arrow_wrap_in_class_method,
    "module_derived_arrow_wrap_in_class_method",
    [prod, dev, ssr, ssr_dev]
);

compiler_module_case!(
    module_ts_strip,
    "module_ts_strip",
    [prod, dev, ssr, ssr_dev]
);

compiler_module_case!(
    module_derived_arrow_wrap_no_state_deps,
    "module_derived_arrow_wrap_no_state_deps",
    [prod, dev, ssr, ssr_dev]
);

compiler_module_case!(
    diagnose_state_private_constructor_init_from_decl_only,
    "diagnose_state_private_constructor_init_from_decl_only",
    [prod, dev, ssr, ssr_dev]
);

compiler_module_case!(
    state_private_class_field_array_proxy,
    "state_private_class_field_array_proxy",
    [prod, dev, ssr, ssr_dev]
);

compiler_module_case!(
    diagnose_module_leading_jsdoc_dropped,
    "diagnose_module_leading_jsdoc_dropped",
    [prod, dev, ssr, ssr_dev]
);

compiler_module_case!(
    ts_strip_non_null_chain,
    "ts_strip_non_null_chain",
    [prod, dev, ssr, ssr_dev]
);

compiler_module_case!(
    module_dev_state_tag,
    "module_dev_state_tag",
    [prod, dev, ssr, ssr_dev]
);

compiler_module_case!(
    module_dev_derived_tag,
    "module_dev_derived_tag",
    [prod, dev, ssr, ssr_dev]
);

compiler_module_case!(
    module_dev_console_log_wrap,
    "module_dev_console_log_wrap",
    [prod, dev, ssr, ssr_dev]
);

compiler_module_case!(
    diagnose_module_import_between_top_level_statements,
    "diagnose_module_import_between_top_level_statements",
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(script_module_exports, [prod, dev, ssr, ssr_dev]);

compiler_case!(script_module_export_specifiers, [prod, dev, ssr, ssr_dev]);

compiler_case!(script_module_imports, [prod, dev, ssr, ssr_dev]);

compiler_case!(script_module_empty, [prod, dev, ssr, ssr_dev]);

compiler_case!(script_module_runes, [prod, dev, ssr, ssr_dev]);

compiler_case!(script_module_instance_ref, [prod, dev, ssr, ssr_dev]);

compiler_case!(script_module_only, [prod, dev, ssr, ssr_dev]);

compiler_case!(script_module_with_instance, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_options_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    svelte_options_runes_false_override,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(svelte_options_accessors_legacy, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_options_immutable_legacy, [prod, dev, ssr, ssr_dev]);

compiler_case!(immutable_init_ctx_mutable, [prod, dev, ssr, ssr_dev]);

compiler_case!(immutable_init_noctx_immutable, [prod, dev, ssr, ssr_dev]);

compiler_case!(immutable_init_noctx_mutable, [prod, dev, ssr, ssr_dev]);

compiler_case!(immutable_source_init_immutable, [prod, dev, ssr, ssr_dev]);

compiler_case!(immutable_source_noinit_immutable, [prod, dev, ssr, ssr_dev]);

compiler_case!(immutable_source_noinit_mutable, [prod, dev, ssr, ssr_dev]);

compiler_case!(immutable_reactive_immutable, [prod, dev, ssr, ssr_dev]);

compiler_case!(legacy_export_let_required, [prod, dev, ssr, ssr_dev]);

compiler_case!(legacy_export_var_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(legacy_export_specifier, [prod, dev, ssr, ssr_dev]);

compiler_case!(legacy_export_specifier_alias, [prod, dev, ssr, ssr_dev]);

compiler_case!(legacy_export_destructure, [prod, dev, ssr, ssr_dev]);

compiler_case!(legacy_export_let_typed, [prod, dev, ssr, ssr_dev]);

compiler_case!(legacy_export_let_member_mutation, [prod, dev, ssr, ssr_dev]);

compiler_case!(legacy_export_let_bind_to_inner, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_legacy_export_let_element_bind_this,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_bind_group_radio_export_let_prop,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_bind_group_radio_attr_update_order,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_bind_group_value_use_action_cache_var_order,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_bind_group_radio_value_shorthand_spread,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_bind_group_input_value_merges_with_text,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_bind_group_radio_value_index_no_cache,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_input_bind_spread_omits_remove_defaults,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_input_spread_only_should_remove_defaults,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_input_boolean_value_emits_remove_defaults,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_input_default_value_attribute_omits_remove_defaults,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_spread_bind_this_action_order,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_attr_expression_restprops_member_coarse_wrap,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_attribute_effect_expression_restprops_coarse_wrap,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_restprops_template_only_function_param,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_child_bind_after_parent_spread_event_order,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_bind_value_on_implicit_reactive_declaration,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_script_let_multi_declarator_splits,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_script_empty_named_import_normalized_to_bare,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_script_ts_empty_named_import_normalized_to_bare,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_bind_value_textarea_export_let_prop,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_bind_files_export_let_prop,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_textarea_bind_value_with_spread_init_order,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_textarea_spread_only_emits_remove_child,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_textarea_dynamic_value_attribute_emits_remove_child,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_export_let_store_prop_subscription,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_export_let_store_prop_writes,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_export_let_leaks_into_exports,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_runes_prop_export_specifier,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(runes_prop_export_specifier_alias, [prod, dev, ssr, ssr_dev]);

compiler_case!(runes_prop_export_specifier_dev, [prod, dev, ssr, ssr_dev]);

compiler_case!(runes_state_export_specifier, [prod, dev, ssr, ssr_dev]);

compiler_case!(runes_raw_state_export_specifier, [prod, dev, ssr, ssr_dev]);

compiler_case!(runes_local_let_export_specifier, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    runes_local_const_export_specifier,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(runes_derived_export_specifier, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_legacy_export_const_init_order,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_props_spread_no_init,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_props_spread_only_no_push,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_export_const_excluded_from_rest_props,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_export_const_bind_prop_after_template_render,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_css_attribute_selector_quote_style,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_css_adjacent_sibling_across_if_block,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_css_attribute_selector_unquoted_value,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_css_declaration_value_comment,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_css_type_class_selector_on_svelte_element,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_component_bind_store_prop,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_component_bind_store_reactive_destructured_base,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_export_let_compound_assign_prop,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_export_let_update_prop_in_template,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_export_let_assign_prop_in_template,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_state_member_update_in_template,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_state_member_compound_in_template,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_export_let_member_update_in_template,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_export_let_member_assign_computed_member_key,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_export_let_default_typed_cast_arrow,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_export_let_default_prop_reference,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_export_let_default_prop_reference_in_conditional,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_export_let_closure_capture_needs_push_init_pop,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_key_prop_call_needs_push_init_pop,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    svelte_component_legacy_derived_hoist_block,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_export_let_key_block_member_coarse_wrap,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_pre_effect_store_subscription_dep,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_reactive_ts_type_position_dep,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_reactive_array_destructure_with_store,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_reactive_assignment_object_destructure_with_store,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_reactive_object_destructure_single_store_leaf,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_reactive_assignment_mixed_destructure,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_reactive_destructure_identifier_rhs,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    runes_prop_member_update_in_template,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    runes_prop_member_compound_in_template,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    smoke_legacy_reactive_mutations_all,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    smoke_runes_reactive_mutations_all,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    smoke_legacy_contextual_mutations_all,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(smoke_legacy_rune_fallback_all, [prod, dev, ssr, ssr_dev]);

compiler_case!(derived_non_runes_invalid_usage, [prod, dev, ssr, ssr_dev]);

compiler_case!(smoke_runes_declarator_gaps_all, [prod, dev, ssr, ssr_dev]);

compiler_case!(smoke_runes_state_eager_panic, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    smoke_ts_non_null_assertion_mutations,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    text_expression_binary_no_nullish_fallback,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    text_expression_conditional_memoized_needs_nullish_fallback,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    svelte_options_preserve_whitespace,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    preserve_whitespace_compile_option_true,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    preserve_whitespace_pre_first_newline,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    preserve_whitespace_inner_trailing_text,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    preserve_whitespace_script_element,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_svg_fragment_root_ws_between_siblings,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_svg_block_fragment_ws_between_siblings,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_svg_component_slot_ws_between_siblings,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_svg_text_block_ws_preserved,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_svg_snippet_body_ws_between_siblings,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_svg_legacy_slot_ws_between_siblings,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_svg_root_html_tag_strategy,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_svg_root_block_only_html_tag,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(preserve_comments_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(preserve_comments_only_child, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    preserve_comments_between_elements,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(preserve_comments_in_block, [prod, dev, ssr, ssr_dev]);

compiler_case!(preserve_comments_svelte_ignore, [prod, dev, ssr, ssr_dev]);

compiler_case!(preserve_comments_only_in_block, [prod, dev, ssr, ssr_dev]);

compiler_case!(preserve_comments_consecutive, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_consecutive_comments_between_components,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(preserve_comments_empty, [prod, dev, ssr, ssr_dev]);

compiler_case!(preserve_comments_in_each, [prod, dev, ssr, ssr_dev]);

// ---------------------------------------------------------------------------
// svelte:head tests
// ---------------------------------------------------------------------------

compiler_case!(svelte_head_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_head_reactive, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_head_with_content, [prod, dev, ssr, ssr_dev]);

// <title> in <svelte:head> tests
compiler_case!(title_variants, [prod, dev, ssr, ssr_dev]);

compiler_case!(async_title_basic, [prod, dev, ssr, ssr_dev]);

// svelte:window tests
compiler_case!(svelte_window_event_legacy, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    svelte_window_event_legacy_with_if,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_svelte_window_on_directive_legacy_prop_handler_wraps,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_svelte_document_on_directive_legacy_prop_handler_wraps,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_svelte_body_on_directive_legacy_prop_handler_wraps,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(svelte_window_action, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_document_action, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_window_event_attr, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    svelte_window_event_attr_props_handler,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    svelte_document_event_attr_props_handler,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    svelte_body_event_attr_props_handler,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(svelte_window_bind_scroll, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_window_bind_size, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_window_bind_size_legacy, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    svelte_window_bind_size_with_template,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(svelte_window_bind_online, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_window_combined, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_window_reactive, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_document_bindings, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_document_events, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_document_bubble, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_document_combined, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_element_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_element_self_closing, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_fragment_named_slot, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    svelte_fragment_named_slot_inside_svelte_component,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    svelte_fragment_named_slot_component_expr_attr,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(component_named_slot_let_fragment, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    component_named_slot_let_fragment_destructure,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    svelte_fragment_default_slot_wrapper,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(svelte_fragment_default_slot_let, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    svelte_fragment_explicit_default_slot_attribute_lowers_to_children_prop,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    svelte_fragment_named_slot_with_const_tag,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(svelte_element_static_tag, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_element_attributes, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_element_spread, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_svelte_element_spread_with_class_directive,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_svelte_element_class_directive_with_dynamic_attr,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_svelte_element_spread_style_directive,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_svelte_element_spread_class_with_bind,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_svelte_element_static_class_with_directive,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_svelte_element_static_style_with_directive,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_svelte_element_dynamic_class_attr_with_directive,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_svelte_element_style_directive_with_dynamic_attr,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_svelte_element_class_and_style_directives,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_scoped_no_class,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_scoped_class_directive,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_scoped_static_class_with_directive,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_scoped_dynamic_class_with_directive,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_class_dynamic_alone,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_class_concat_alone,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_class_concat_with_directive,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_class_array_alone,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_class_array_with_directive,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_style_dynamic_alone,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_style_concat_alone,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_style_dynamic_with_directive,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_scoped_style_directive,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_class_directive_with_bind,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_class_directive_with_use,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_dynamic_attr_with_both_directives,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_xmlns_with_class_directive,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_static_class_with_style_directive,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_static_class_with_both_directives,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_on_legacy_with_class_directive,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_transition_only,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_spread_with_use,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_svelte_element_attribute_effect_call_value_memo,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_svelte_element_use_action_with_on_event_legacy,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_style_directive_with_bind,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_class_directive_expression,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_style_directive_important,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_style_directive_custom_prop,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_animate_in_each,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_bind_this_store,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(audit_svelte_element_attach_tag, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    audit_svelte_element_multiple_class_directives,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_multiple_style_directives,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_bind_this_member,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_multiple_spreads,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_transition_with_params,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_transition_local,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_in_out_separate,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_use_action_with_params,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_modern_event_handler,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_async_tag_with_class_directive,
    [prod, dev_todo, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_async_tag_with_spread,
    [prod, dev_todo, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_scoped_with_spread,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_class_directive_with_call,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(audit_svelte_element_dev_spread, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    audit_svelte_element_dev_class_directive,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_transition_in_with_params,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_transition_global,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_class_directive_with_each,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    audit_svelte_element_dev_static_class_with_directive,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(svelte_element_onclick, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_element_bind, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_element_null_tag, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_element_xmlns, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_element_dynamic_xmlns, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_element_children_expr, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    svelte_element_body_multi_child_fragment,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(svelte_body_event_attr, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_body_event_legacy, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_body_action, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_body_combined, [prod, dev, ssr, ssr_dev]);

compiler_case!(boundary_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(boundary_failed_snippet, [prod, dev, ssr, ssr_dev]);

compiler_case!(boundary_onerror, [prod, dev, ssr, ssr_dev]);

compiler_case!(boundary_pending_snippet, [prod, dev, ssr, ssr_dev]);

compiler_case!(boundary_failed_onerror, [prod, dev, ssr, ssr_dev]);

compiler_case!(boundary_failed_attribute, [prod, dev, ssr, ssr_dev]);

compiler_case!(boundary_all_three, [prod, dev, ssr, ssr_dev]);

compiler_case!(boundary_reactive_onerror, [prod, dev, ssr, ssr_dev]);

compiler_case!(boundary_nested, [prod, dev, ssr, ssr_dev]);

compiler_case!(boundary_const_tag, [prod, dev, ssr, ssr_dev]);

compiler_case!(boundary_in_if, [prod, dev, ssr, ssr_dev]);

compiler_case!(boundary_other_snippets, [prod, dev, ssr, ssr_dev]);

compiler_case!(boundary_pending_attribute, [prod, dev, ssr, ssr_dev]);

compiler_case!(boundary_pending_imported, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    boundary_failed_attribute_override,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    boundary_pending_attribute_override,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(await_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(await_short_then, [prod, dev, ssr, ssr_dev]);

compiler_case!(await_short_catch, [prod, dev, ssr, ssr_dev]);

compiler_case!(await_then_catch, [prod, dev, ssr, ssr_dev]);

compiler_case!(await_no_bindings, [prod, dev, ssr, ssr_dev]);

compiler_case!(await_pending_only, [prod, dev, ssr, ssr_dev]);

compiler_case!(await_destructured, [prod, dev, ssr, ssr_dev]);

compiler_case!(await_in_if, [prod, dev, ssr, ssr_dev]);

compiler_case!(await_in_each, [prod, dev, ssr, ssr_dev]);

compiler_case!(await_reactive, [prod, dev, ssr, ssr_dev]);

compiler_case!(await_nested_content, [prod, dev, ssr, ssr_dev]);

// ---------------------------------------------------------------------------
// Event attribute tests (Svelte 5)
// ---------------------------------------------------------------------------

compiler_case!(event_attr_non_delegatable, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    event_attr_delegated_after_non_delegated_order,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(event_attr_capture, [prod, dev, ssr, ssr_dev]);

compiler_case!(event_attr_capture_non_deleg, [prod, dev, ssr, ssr_dev]);

compiler_case!(event_attr_gotpointercapture, [prod, dev, ssr, ssr_dev]);

compiler_case!(event_attr_passive, [prod, dev, ssr, ssr_dev]);

compiler_case!(event_attr_passive_window, [prod, dev, ssr, ssr_dev]);

compiler_case!(event_attr_import_handler, [prod, dev, ssr, ssr_dev]);

compiler_case!(event_attr_member_handler, [prod, dev, ssr, ssr_dev]);

compiler_case!(event_attr_const_tag_destructure, [prod, dev, ssr, ssr_dev]);

compiler_case!(event_attr_props_handler, [prod, dev, ssr, ssr_dev]);

compiler_case!(event_attr_has_call, [prod, dev, ssr, ssr_dev]);

compiler_case!(event_attr_dev_apply, [prod, dev, ssr, ssr_dev]);

// ---------------------------------------------------------------------------
// Expression memoization tests
// ---------------------------------------------------------------------------

compiler_case!(component_prop_has_call, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_prop_has_call_multi, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_prop_has_call_mixed, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_prop_concat_call_memo, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_component_prop_concat_legacy_ternary,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    component_prop_concat_call_with_literal,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    component_prop_concat_import_identifier,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    component_prop_concat_import_and_call,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(html_concat_call_with_literal, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    html_class_concat_call_with_literal,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(component_dynamic_dotted, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_prop_memo_state, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    component_prop_const_call_init_getter,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    component_prop_const_member_init_getter,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    component_prop_imported_direct_getter,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(component_prop_import_meta_getter, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    element_attr_import_meta_template_effect,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(render_tag_arg_has_call, [prod, dev, ssr, ssr_dev]);

compiler_case!(render_tag_arg_has_call_multi, [prod, dev, ssr, ssr_dev]);

compiler_case!(render_tag_arg_mixed, [prod, dev, ssr, ssr_dev]);

compiler_case!(render_tag_dynamic_prop, [prod, dev, ssr, ssr_dev]);

compiler_case!(render_tag_dynamic_state, [prod, dev, ssr, ssr_dev]);

compiler_case!(render_tag_dynamic_snippet_param, [prod, dev, ssr, ssr_dev]);

compiler_case!(render_tag_optional, [prod, dev, ssr, ssr_dev]);

compiler_case!(render_tag_optional_dynamic, [prod, dev, ssr, ssr_dev]);

// ---------------------------------------------------------------------------
// $inspect rune tests
// ---------------------------------------------------------------------------

compiler_case!(inspect_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(inspect_with_callback, [prod, dev, ssr, ssr_dev]);

compiler_case!(inspect_prod_strip, [prod, dev, ssr, ssr_dev]);

// ---------------------------------------------------------------------------
// $inspect.trace() rune tests
// ---------------------------------------------------------------------------

compiler_case!(inspect_trace_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(inspect_trace_contexts, [prod, dev, ssr, ssr_dev]);

compiler_case!(inspect_trace_prod_strip, [prod, dev, ssr, ssr_dev]);

compiler_case!(inspect_trace_reactive_contexts, [prod, dev, ssr, ssr_dev]);

// ---------------------------------------------------------------------------
// $props.id() rune tests
// ---------------------------------------------------------------------------

compiler_case!(props_id_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(props_id_with_props, [prod, dev, ssr, ssr_dev]);

// ---------------------------------------------------------------------------
// {@debug} tests
// ---------------------------------------------------------------------------

compiler_case!(debug_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(debug_in_blocks, [prod, dev, ssr, ssr_dev]);

// ---------------------------------------------------------------------------
// TypeScript stripping tests
// ---------------------------------------------------------------------------

compiler_case!(ts_strip_expression_tag, [prod, dev, ssr, ssr_dev]);

compiler_case!(ts_strip_satisfies, [prod, dev, ssr, ssr_dev]);

compiler_case!(ts_strip_non_null, [prod, dev, ssr, ssr_dev]);

compiler_case!(ts_strip_const_tag, [prod, dev, ssr, ssr_dev]);

compiler_case!(ts_strip_attribute, [prod, dev, ssr, ssr_dev]);

compiler_case!(ts_strip_script_types, [prod, dev, ssr, ssr_dev]);

compiler_case!(ts_strip_handler_param_annotation, [prod, dev, ssr, ssr_dev]);

compiler_case!(ts_strip_as_paren_optional_chain, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    ts_strip_catch_empty_comment_orphan,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_ts_cast_const_init_marks_attr_dynamic,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(namespace_svg, [prod, dev, ssr, ssr_dev]);

compiler_case!(namespace_mathml, [prod, dev, ssr, ssr_dev]);

compiler_case!(svg_fragment_ambiguous_a, [prod, dev, ssr, ssr_dev]);

compiler_case!(svg_fragment_ambiguous_title, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_element_in_if, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_element_class_directive, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_element_style_directive, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_element_dev_invalid_tag, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_element_dev_void_children, [prod, dev, ssr, ssr_dev]);

compiler_case!(boundary_const_in_snippet, [prod, dev, ssr, ssr_dev]);

compiler_case!(boundary_imported_handler, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_this_sequence, [prod, dev, ssr, ssr_dev]);

// ---------------------------------------------------------------------------
// Tier 2b — Template Tags
// ---------------------------------------------------------------------------

compiler_case!(await_array_destructured, [prod, dev, ssr, ssr_dev]);

compiler_case!(html_tag_controlled, [prod, dev, ssr, ssr_dev]);

compiler_case!(html_tag_svg, [prod, dev, ssr, ssr_dev]);

compiler_case!(html_tag_nested_svg, [prod, dev, ssr, ssr_dev]);

compiler_case!(html_tag_nested_mathml, [prod, dev, ssr, ssr_dev]);

compiler_case!(html_tag_hydration_ignore, [prod, dev, ssr, ssr_dev]);

compiler_case!(const_tag_dev, [prod, dev, ssr, ssr_dev]);

compiler_case!(rune_compound_template, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_assign_template, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_compound_template, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_update_template, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_deep_mutation, [prod, dev, ssr, ssr_dev]);

compiler_case!(store_deep_update, [prod, dev, ssr, ssr_dev]);

// ---------------------------------------------------------------------------
// Tier 2c — Bind Directive Edge Cases
// ---------------------------------------------------------------------------

compiler_case!(bind_function_value, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_function_checked, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_use_deferral, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_contenteditable_flag, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_group_each, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_group_keyed_each, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_group_nested_each, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_group_value_attr, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_group_value_attr_before_bind, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    bind_group_each_legacy_item_member_untrack,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(bind_group_each_var, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_group_each_var_keyed, [prod, dev, ssr, ssr_dev]);

compiler_case!(each_fallback, [prod, dev, ssr, ssr_dev]);

compiler_case!(each_keyed_index, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_each_key_legacy_prop_no_rewrite,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(each_keyed_index_plain_in_body, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    each_key_is_index_literal_diagnose,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(each_key_uses_index, [prod, dev, ssr, ssr_dev]);

compiler_case!(each_key_is_item, [prod, dev, ssr, ssr_dev]);

compiler_case!(each_destructured_obj, [prod, dev, ssr, ssr_dev]);

compiler_case!(each_destructured_default, [prod, dev, ssr, ssr_dev]);

compiler_case!(each_destructured_array, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_each_rest_only_pattern_binding,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(each_destructured_obj_with_rest, [prod, dev, ssr, ssr_dev]);

compiler_case!(each_destructured_array_with_rest, [prod, dev, ssr, ssr_dev]);

compiler_case!(each_destructured_array_rest_only, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_legacy_each_component_css_prop_hoist_derived,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(style_attr_object, [prod, dev, ssr, ssr_dev]);

compiler_case!(style_attr_dynamic, [prod, dev, ssr, ssr_dev]);

compiler_case!(script_jsdoc_comment, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_head_title_meta, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    snippet_ident_conflict_with_script,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(debug_non_dev, [prod, dev, ssr, ssr_dev]);

compiler_case!(debug_non_runes_untrack, [prod, dev, ssr, ssr_dev]);

compiler_case!(non_runes_simple_snapshot, [prod, dev, ssr, ssr_dev]);

compiler_case!(animate_with_spread, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_element_static_class_attr, [prod, dev, ssr, ssr_dev]);

compiler_case!(root_with_special_elements, [prod, dev, ssr, ssr_dev]);

compiler_case!(needs_context_method_chain, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    event_handler_derived_with_class_directives,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    event_handler_derived_with_class_object,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_class_directive_legacy_event_handler_derived_order,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_class_directive_named_slot_let_classes_order_legacy,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(derived_inside_function, [prod, dev, ssr, ssr_dev]);

compiler_case!(derived_nested_getter, [prod, dev, ssr, ssr_dev]);

compiler_case!(derived_shorthand_property, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_inside_function, [prod, dev, ssr, ssr_dev]);

compiler_case!(derived_by_inside_function, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_snippet_prop, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_snippet_with_children, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_multiple_snippets, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_snippet_only, [prod, dev, ssr, ssr_dev]);

// ---------------------------------------------------------------------------
// Diagnose: TypeScript import + spread + bind:prop tests
// ---------------------------------------------------------------------------

compiler_case!(ts_type_import_comment, [prod, dev, ssr, ssr_dev]);

compiler_case!(rest_props_member_access, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_spread_props, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_bind_prop_forward, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_bind_member_path, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_bind_member_path_bindable_root, [prod, dev_todo]);

compiler_case!(component_bind_member_path_dev, [prod, dev, ssr, ssr_dev]);

compiler_case!(component_bind_function, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    component_bind_function_anchor_order,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_bind_function_props_position,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_bind_derived_target_no_proxy_flag,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_fragment_id_after_component_with_snippet,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_fragment_id_in_snippet_used_as_expression,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_fragment_id_in_sibling_named_slot_after_component,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(component_prop_const_tag_member, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_component_prop_const_tag_member_init,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_prop_const_tag_destructured_shorthand,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_each_const_tag_shorthand_prop_to_component,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_const_tag_legacy_dependency_destructure,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_const_tag_legacy_dep_read_spread_of_derived,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_slot_let_array_destructure_dep_read,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_svelte_fragment_let_inside_named_slot_component,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_svelte_component_css_custom_prop_wrapper,
    [prod, dev_todo]
);

compiler_case!(
    diagnose_component_prop_computed_member_getter,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_prop_hyphenated_key_derived,
    [prod, dev, ssr, ssr_dev]
);

// ---------------------------------------------------------------------------
// Diagnose: svelte import patterns
// ---------------------------------------------------------------------------

compiler_case!(needs_context_nested_fn, [prod, dev, ssr, ssr_dev]);

compiler_case!(member_expr_dynamic_local, [prod, dev, ssr, ssr_dev]);

compiler_case!(import_type_mixed, [prod, dev, ssr, ssr_dev]);

compiler_case!(derived_in_nested_function, [prod, dev, ssr, ssr_dev]);

compiler_case!(derived_local_signal_get, [prod, dev, ssr, ssr_dev]);

compiler_case!(svelte_element_duplicate_naming, [prod, dev, ssr, ssr_dev]);

compiler_case!(each_block_no_item, [prod, dev, ssr, ssr_dev]);

compiler_case!(each_block_no_item_multi, [prod, dev, ssr, ssr_dev]);

compiler_case!(each_block_no_item_with_index, [prod, dev, ssr, ssr_dev]);

compiler_case!(each_collection_call_reads_state, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    legacy_each_collection_member_const_wraps_untrack,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_each_collection_member_reactive_let_wraps_with_read,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_collection_member_nullish_fallback_wraps_with_read,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_collection_member_outer_each_item_wraps,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_collection_member_export_let_prop_wraps,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_collection_member_imported_wraps_with_read,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_call_imported_wraps,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_call_imported_args_wrap,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_call_local_fn_wraps,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_collection_new_expression_wraps,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_collection_new_expression_prop_arg_wraps,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(async_if_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(async_if_else_if_condition, [prod, dev, ssr, ssr_dev]);

compiler_case!(if_elseif_new_blockers, [prod, dev, ssr, ssr_dev]);

compiler_case!(async_each_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(async_html_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(async_key_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(async_await_has_await, [prod, dev, ssr, ssr_dev]);

compiler_case!(async_flag_import, [prod, dev, ssr, ssr_dev]);

compiler_case!(async_blockers_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(async_bind_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(action_blockers, [prod, dev, ssr, ssr_dev]);

compiler_case!(attach_blockers, [prod, dev, ssr, ssr_dev]);

compiler_case!(transition_blockers, [prod, dev, ssr, ssr_dev]);

compiler_case!(animate_blockers, [prod, dev, ssr, ssr_dev]);

compiler_case!(async_svelte_element, [prod, dev_todo, ssr, ssr_dev]);

compiler_case!(async_const_tag, [prod, dev, ssr, ssr_dev]);

compiler_case!(async_derived_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(async_derived_destructured, [prod, dev, ssr, ssr_dev]);

compiler_case!(async_derived_dev, [prod, dev, ssr, ssr_dev]);

compiler_case!(async_derived_dev_ignored, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    async_derived_dev_ignored_destructured,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(async_derived_nested_function, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    async_derived_nested_function_destructured,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(async_for_await_dev, [prod, dev, ssr, ssr_dev]);

compiler_case!(orthogonality_heavy_call, [prod, dev, ssr, ssr_dev]);

compiler_case!(orthogonality_async_await, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    orthogonality_heavy_async_await_call,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(inline_await_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(inline_await_global_callee, [prod, dev, ssr, ssr_dev]);

compiler_case!(inline_await_text_concat, [prod, dev, ssr, ssr_dev]);

compiler_case!(inline_await_attr, [prod, dev, ssr, ssr_dev]);

compiler_case!(async_pickled_await_template, [prod, dev, ssr, ssr_dev]);

compiler_case!(async_render_tag, [prod, dev, ssr, ssr_dev]);

compiler_case!(async_render_tag_complex_args, [prod, dev, ssr, ssr_dev]);

compiler_case!(async_boundary_const, [prod, dev, ssr, ssr_dev]);

compiler_case!(async_const_derived_chain, [prod, dev, ssr, ssr_dev]);

compiler_case!(attach_on_component, [prod, dev, ssr, ssr_dev]);

compiler_case!(attach_on_component_dynamic, [prod, dev, ssr, ssr_dev]);

compiler_case!(attach_on_document, [prod, dev, ssr, ssr_dev]);

compiler_case!(each_keyed_destructure, [prod, dev, ssr, ssr_dev]);

compiler_case!(await_then_text_before_element, [prod, dev, ssr, ssr_dev]);

compiler_case!(await_thunk_optimization, [prod, dev, ssr, ssr_dev]);

compiler_case!(await_each_nested, [prod, dev, ssr, ssr_dev]);

compiler_case!(await_pending_then, [prod, dev, ssr, ssr_dev]);

compiler_case!(await_pending_catch, [prod, dev, ssr, ssr_dev]);

compiler_case!(await_short_catch_no_binding, [prod, dev, ssr, ssr_dev]);

compiler_case!(await_nested_await, [prod, dev, ssr, ssr_dev]);

compiler_case!(fragment_counter_with_nested_if, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_group_radio_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_multiple_on_element, [prod, dev, ssr, ssr_dev]);

compiler_case!(if_else_chain_with_const, [prod, dev, ssr, ssr_dev]);

compiler_case!(event_mixed_delegation, [prod, dev, ssr, ssr_dev]);

compiler_case!(props_identifier_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(props_identifier_await_expression, [prod, dev, ssr, ssr_dev]);

compiler_case!(call_expr_local_method_dynamic, [prod, dev, ssr, ssr_dev]);

compiler_case!(call_expr_nested_fn_dynamic, [prod, dev, ssr, ssr_dev]);

compiler_case!(effect_cleanup_return, [prod, dev, ssr, ssr_dev]);

compiler_case!(tag_derived_basic, [prod, dev, ssr, ssr_dev]);

compiler_case!(tag_derived_by, [prod, dev, ssr, ssr_dev]);

compiler_case!(derived_destructured_object, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    derived_destructured_object_prop_source,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    derived_destructured_props_whole_source,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(derived_destructured_array, [prod, dev, ssr, ssr_dev]);

compiler_case!(derived_destructured_by, [prod, dev, ssr, ssr_dev]);

compiler_case!(tag_state_unmutated, [prod, dev, ssr, ssr_dev]);

compiler_case!(tag_state_unmutated_no_proxy, [prod, dev, ssr, ssr_dev]);

compiler_case!(tag_bindable_proxy, [prod, dev, ssr, ssr_dev]);

compiler_case!(tag_class_field_public, [prod, dev, ssr, ssr_dev]);

compiler_case!(tag_class_field_private, [prod, dev, ssr, ssr_dev]);

compiler_case!(tag_class_constructor, [prod, dev, ssr, ssr_dev]);

compiler_case!(tag_snippet_dev, [prod, dev, ssr, ssr_dev]);

compiler_case!(tag_render_dev, [prod, dev, ssr, ssr_dev]);

compiler_case!(snippet_destructure_dev, [prod, dev, ssr, ssr_dev]);

compiler_case!(snippet_object_destructure, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_legacy_snippet_param_member_to_component_prop,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_component_prop_import_meta_getter_in_if,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_snippet_inside_each_callback,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(snippet_array_destructure, [prod, dev, ssr, ssr_dev]);

compiler_case!(snippet_mixed_params, [prod, dev, ssr, ssr_dev]);

compiler_case!(snippet_nested_destructure, [prod, dev, ssr, ssr_dev]);

compiler_case!(snippet_computed_key_destructure, [prod, dev, ssr, ssr_dev]);

compiler_case!(snippet_body_single_component, [prod, dev, ssr, ssr_dev]);

compiler_case!(tag_state_destructured_array, [prod, dev, ssr, ssr_dev]);

compiler_case!(tag_state_destructured_object, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_var_safe_get, [prod, dev, ssr, ssr_dev]);

compiler_case!(state_assign_dev, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_scoped_class_selector, [prod, dev, ssr, ssr_dev]);

compiler_case!(bind_select_static_option_value, [prod, dev, ssr, ssr_dev]);

compiler_case!(input_dynamic_special_attrs, [prod, dev, ssr, ssr_dev]);

compiler_case!(svg_dynamic_special_attrs, [prod, dev, ssr, ssr_dev]);

compiler_case!(each_index_text_no_coalesce, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    snippet_destructure_default_state_ref,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    snippet_destructure_default_mutated_state_ref,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_props_bindable_icon_component,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_props_identifier_in_snippet_body,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_filename_name_collides_with_ts_type_import,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    props_bindable_checkbox_disabled_shorthand_ts,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(diagnose_component_onclick_state, [prod, dev, ssr, ssr_dev]);

compiler_case!(diagnose_svg_city_icon);

compiler_case!(clock_svg_derived_onmount, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_component_default_and_named_slot_expr,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_named_slot_child_with_expression_prop,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_named_slot_child_inflates_template_root_ids,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_named_slot_empty_element_child_kept,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_named_slot_svelte_component_child_kept,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_svelte_component_named_slot_expression_binds,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_slot_forward_inflates_template_root_ids,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(diagnose_legacy_dev_benchmark, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    component_dev_default_children_wrap_snippet,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    component_dev_named_slot_no_wrap_snippet,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(diagnose_dev_benchmark, [prod, dev, ssr, ssr_dev]);

compiler_case!(dev_binary_equals_wrap, [prod, dev, ssr, ssr_dev]);

compiler_case!(dev_filename_relative_to_root_dir, [prod, dev, ssr, ssr_dev]);

compiler_case!(dev_filename_root_dir_no_match, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    add_locations_svelte_head_skips_hoisted_title,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(add_locations_named_slot_wrapper, [prod, dev, ssr, ssr_dev]);

compiler_case!(auto_softlegacy_member_read, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    auto_hardlegacy_member_read_explicit,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    auto_hardlegacy_store_autosub_shadows_rune,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    auto_hardlegacy_import_call_coarse_wrap,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(legacy_dev_inspect_fallback, [prod, dev, ssr, ssr_dev]);

compiler_case!(legacy_dev_const_deep_read_wrap, [prod, dev, ssr, ssr_dev]);

compiler_case!(legacy_dev_each_const_deep_read, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    legacy_dev_attribute_effect_grouping,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_dev_component_event_prop_derived,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(legacy_dev_component_prop_getter, [prod, dev, ssr, ssr_dev]);

compiler_case!(auto_softlegacy_simple_template, [prod, dev, ssr, ssr_dev]);

compiler_case!(auto_softlegacy_const_only, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    auto_softlegacy_const_member_mutation,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_state_bind_member_mutate_wrap,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    auto_softlegacy_component_prop_derived,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(auto_softlegacy_render_arg_memo, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    auto_softlegacy_template_call_expr,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    needs_context_template_member_in_binary,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    needs_context_template_member_in_conditional,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    needs_context_template_member_root_import,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    needs_context_template_member_ts_props,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_bind_group_emits_array,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_html_template_preserves_nbsp,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_attribute_entity_decoding,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_attribute_entity_strict_mode,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_onclick_const_arrow,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_spread_call_memo,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_button_single_dynamic_text,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_text_concat_import_uses_template_effect,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_text_concat_sequence_expr_nullish_fallback,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_static_text_before_dynamic_in_element,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_snippet_inside_if_consequent,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_snippet_hoistable_with_script_import,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_snippet_hoistable_param_type_annotation,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_snippet_name_with_underscore,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_hoisted_snippet_module_order_with_sibling_template,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_snippet_store_autosub_not_hoistable,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_on_directive_shorthand_forward,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_pre_effect_order_after_functions,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_template_effect_prop_call_coarse_wrap,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_local_var_not_promoted_to_state,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_reactive_assignment_promotes_state_via_handler,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_reactive_arrow_value_promotes_state,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_const_destructure_coarse_wrap,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_if_condition_coarse_wrap,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_html_member_coarse_wrap,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_class_attribute_prop_member_coarse_wrap,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_bind_this_indexed_reactive,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_bind_this_indexed_by_index_variable,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_store_bind_value_item_member,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_store_bind_checked_item_member,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_store_bind_group_item_member,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_store_bind_value_element_item_member,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_store_bind_checked_element_item_member,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_video_muted_static_attribute_lowers_to_property,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_video_muted_concatenation_attribute_lowers_to_property,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_if_store_short_circuit_coarse_wrap,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_index_component_prop_plain,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_css_props_member_coarse_wrap,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_css_props_concat_member_coarse_wrap,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_each_css_wrapper_root_order,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_bind_store_derived_base,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_nested_delegated_transition_order,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_text_entity_leading_ws_before_if_block,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_td_sibling_if_blocks_whitespace,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_component_prop_const_call_safe_equal_untrack,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_component_prop_call_with_prop_arg_coarse_wrap,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_component_prop_ternary_arrow_no_coarse_wrap,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_style_directive_literal_skips_template_effect_grouping,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_style_directive_quoted_literal_interp_static,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_style_directive_template_literal_const_ident_fold,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_style_directive_const_ident_fold,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_style_directive_const_number_ident_fold,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_style_directive_const_binary_init_ident_fold,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_style_directive_multiple_const_idents_fold,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_style_directive_const_ident_fold_runes,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_style_directive_const_with_state_keeps_reactive,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_style_directive_legacy_let_no_fold,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_style_directive_complex_expression_hoists_to_memo_deps,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_style_attr_call_groups_in_template_effect_memo,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_template_effect_merge_memo_attr_with_simple_attr,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    attr_call_value_hoists_to_template_effect_memo,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_attr_call_no_references_init,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_style_attr_call_no_references_with_directive_init,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_style_attr_dynamic_with_style_directive_merges_set_style,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_style_attr_with_directive_source_order,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_style_directive_with_static_style_attribute,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(noscript_root_in_if_block, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_legacy_pre_effect_dep_order_switch_externals,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_pre_effect_dep_order_lhs_write_before_prop_read,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_iife_read_nested_write_promotes_state,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_pre_effect_store_write_read_topological_order,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_css_slot_fallback_descendant_scope,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(ts_init_impure_type_assertion, [prod, dev, ssr, ssr_dev]);

compiler_case!(ts_class_field_rune_paren, [prod, dev, ssr, ssr_dev]);

compiler_case!(ts_class_field_rune_as, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    ts_constructor_this_rune_assignment_ts,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(ts_bindable_default_as, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    ts_bindable_default_identifier_paren,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(ts_derived_async_await_paren, [prod, dev, ssr, ssr_dev]);

compiler_case!(ts_state_proxyable_paren_arrow, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    ts_simple_expression_paren_default,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(ts_each_key_is_index_paren, [prod, dev, ssr, ssr_dev]);

compiler_case!(ts_each_key_is_item_as, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    ts_legacy_reactive_assign_satisfies,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_let_reassigned_unread_stays_plain,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_export_const_emits_bind_prop,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    component_name_ignores_ts_namespace_type,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_on_directive_legacy_reactive_handler,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_reactive_store_value_passed_as_bare_prop,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_let_writable_store_only_assign,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_bind_store_member_keeps_writable_plain,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_component_bind_base_store_unsub,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_prop_call_fold_global_inlined,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_ts_type_param_node_skews_anchor_idents,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_prop_const_init_identifier_inline,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_slot_attribute_const_arrow_shorthand,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_prop_object_literal_arrow_value_inline,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_prop_object_literal_shorthand_inline,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_name_collides_with_slot_let_binding,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_name_collides_with_each_alias_binding,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_component_name_collides_with_snippet_param_binding,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_collection_store_member_optional_wraps,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_soft_legacy_each_store_member_emits_boot_scaffolding,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_soft_legacy_each_store_item_reactive_read,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_each_collection_store_member_local_const_no_wrap,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_each_collection_logical_nullish,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_each_collection_conditional,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(legacy_each_collection_logical_or, [prod, dev, ssr, ssr_dev]);

compiler_case!(legacy_each_collection_sequence, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    legacy_each_collection_parenthesized,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    each_collection_source_prop_direct,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    each_collection_source_prop_parenthesized,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    each_collection_source_local_member_chain,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(render_tag_optional_chain, [prod, dev, ssr, ssr_dev]);

compiler_case!(render_tag_conditional_callee, [prod, dev, ssr, ssr_dev]);

compiler_case!(render_tag_logical_callee, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    render_tag_args_with_reactive_state,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(legacy_await_member_chain_promise, [prod, dev, ssr, ssr_dev]);

compiler_case!(legacy_await_call_with_state_arg, [prod, dev, ssr, ssr_dev]);

compiler_case!(
    diagnose_css_custom_prop_component_concat_literal_const_fold,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_slot_element_let_directive_alias_in_named_slot_fill,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_on_directive_optional_chain_handler_wrap,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    diagnose_legacy_slot_let_const_tag_ordering,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(
    legacy_bindable_export_with_api_export,
    [prod, dev, ssr, ssr_dev]
);

compiler_case!(css_has_leading_plus_global, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_has_leading_plus_global_flat, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_has_leading_child_global, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_has_leading_plus_class, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_has_descendant_global, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_has_leading_tilde_global, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_nth_child_leading_plus_sign, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_nth_child_of_selector_list, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_nth_child_of_combinator, [prod, dev, ssr, ssr_dev]);

compiler_case!(css_nth_child_uppercase_name, [prod, dev, ssr, ssr_dev]);
