...
var root = $.from_html(`<meta name="description" content="Benchmark component" class="svelte-13nvtxg"/> <link rel="canonical" href="/benchmark" class="svelte-13nvtxg"/>`, 1);
var root_1 = $.from_html(`<span> </span>`);
var root_2 = $.from_html(`<div class="card svelte-13nvtxg"><h3 class="svelte-13nvtxg"> </h3> <p class="svelte-13nvtxg"> </p> <!></div>`);
var root_3 = $.from_html(`<span class="svelte-13nvtxg"> </span>`);
var root_4 = $.from_html(`<section class="summary svelte-13nvtxg"><h4 class="svelte-13nvtxg"> </h4> <!></section>`);
var root_5 = $.from_html(`<span empty="" class="svelte-13nvtxg"> </span>`);
var root_6 = $.from_html(`<h1 class="svelte-13nvtxg">Lorem ipsum dolor sit amet. Chunk 0.</h1>`);
var root_7 = $.from_html(`<h2 class="svelte-13nvtxg">EMPTY</h2>`);
var root_8 = $.from_html(`<div class="svelte-13nvtxg"><input class="svelte-13nvtxg"/></div> <!>`, 1);
var root_9 = $.from_html(`<p class="svelte-13nvtxg"> </p>`);
var root_10 = $.from_html(`<p> </p>`);
var root_11 = $.from_html(`<span class="item-less svelte-13nvtxg">Repeated shell chunk 0</span>`);
var root_12 = $.from_html(`<p class="svelte-13nvtxg">Loading chunk 0...</p>`);
var root_13 = $.from_html(`<strong class="svelte-13nvtxg"> </strong>`);
var root_14 = $.from_html(`<div slot="footer" class="svelte-13nvtxg"> </div>`);
var root_15 = $.from_html(`<div class="chunk-shell benchmark-reset benchmark-host svelte-13nvtxg" data-kind="chunk-0"> <p class="svelte-13nvtxg"> </p> <p class="svelte-13nvtxg"> </p> <!> <div>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod
...
export default function App($$anchor, $$props) {
	...
	const metricSummary = ($$anchor, $$arg0) => {
		...
		$.each(node_2, 1, () => $.get(values), $.index, ($$anchor, value, index) => {
			var span_1 = root_3();
	...
	let state = $.mutable_source($state()(""));
	let counter = $.mutable_source($state()(0));
	let rawData = $state().raw({
		x: 1,
		y: 2
	});
	let checked = $.mutable_source($state()(false));
	let group = $.mutable_source($state()([]));
	let volume = $.mutable_source($state()(.5));
	let selected = $.mutable_source($state()("opt-0"));
	...
	$.each(node_7, 11, () => items, (item) => item.id, ($$anchor, item, idx) => {
		const itemLabel = $.derived_safe_equal(() => ($.deep_read_state($.get(idx)), $.get(item), $.untrack(() => `${$.get(idx)}:${$.get(item).name}`)));
		var p_4 = root_10();
	...
	$.each(node_8, 1, () => items, $.index, ($$anchor, $$item) => {
		var span_3 = root_11();
	...
	var node_12 = $.sibling(node_11, 2);
	{
		...
		$.bind_this(ChildComponent(node_12, {
			...
			$$legacy: true
		}), ($$value) => $.set(componentRef, $$value), () => $.get(componentRef));
	}
	...
	$.bind_value(input_1, () => $.get(state), ($$value) => $.store_unsub($.set(state, $$value), "$state", $$stores));
	$.bind_value(textarea, () => $.get(state), ($$value) => $.store_unsub($.set(state, $$value), "$state", $$stores));
	...
	$.bind_content_editable("innerHTML", div_4, () => $.get(state), ($$value) => $.store_unsub($.set(state, $$value), "$state", $$stores));
