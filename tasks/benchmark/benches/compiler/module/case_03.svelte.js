...
import { OnlineControlResponseDecision, AGENTResult } from "$gatewayService/__queryTypes";
...
import { EndpointBTransferResult, Steps } from "../types";
export const createStore = (initState) => {
	const state = $state(initState);
	const fmt = moneyFormatterFactory();
	const setSelectedTable = (direction, key) => {
		if (direction === "from") {
			...
			if (!targets.includes(currentSelectedToKey)) {
				state.selectedSources.to = state.sourcesByKey.get(targets[0] ?? "");
			}
			state.sourcesTo = targets.map((key) => state.sourcesByKey.get(key)).filter((item) => item?.type === "table");
			state.selectedSources.from = state.sourcesByKey.get(key);
			if (state.cents) validateAmount(state.cents);
		} else {
			state.selectedSources.to = state.sourcesByKey.get(key);
		}
	};
	...
	const validateAmount = (amount) => {
		...
		if (state.minTransferAmount && amount < state.minTransferAmount) {
			state.error = i18n("mfTransferBetween.amount.error.minTransferAmount", "lorem lorem lorem lorem lorem {minAmount}", { minAmount: fmt(state.minTransferAmount) });
			return false;
		}
		if (from.type === "tree") {
			...
			if (tableService.isGoogleTable(from.productTypeV3)) {
				...
				if (amount < minLimit) {
					state.error = i18n("mfTransferBetween.amount.error.googleMinTransfer", "lorem lorem lorem {minLimit}", { minLimit: fmt(minLimit) });
					return false;
				}
			}
		}
		if (tableService.isGoogleTable(to.productTypeV3)) {
			...
			if (fullDebt > 0 && amount > fullDebt) {
				state.error = i18n("mfTransferBetween.amount.error.googleMaxTransfer", "lorem lorem lorem {maxLimit}", { maxLimit: fmt(fullDebt) });
				return false;
			}
		}
		return true;
	...
	const showSuccessTransferCurtain = async (from, to, amount, operationInfo) => {
		if (state.isPapperTicket) return;
		const operationPolling = operationInfo ? await pollOperation({
			groupID: operationInfo.groupOperationID,
			groupOperationType: operationInfo.groupOperationType
		}) : undefined;
	...
	const showFailTransferNotice = () => {
		eventBus.snackbarV2.alert(i18n("mfTransferBetween.transfer.error.message", "lorem lorem lorem lorem"), i18n("mfTransferBetween.transfer.error.description", "lorem, lorem lorem lorem"));
	};
	const showEnablePushNotificationCurtain = (toTableType) => {
	...
	const enableAgent = async () => {
		const result = await apiClient.clientRules_create.call({ input: {
			allow: true,
			groupCode: "agent"
		} });
		if (result.isOk()) {
			...
			eventBus.dispatchEvent({
				name: AppEvents.ENABLE_AGENT,
				payload: null
			});
			eventBus.snackbarV2.success(i18n("mfTransferBetween.enableAgent.snackbar.success.title", "lorem lorem{NBSP}lorem lorem lorem"));
			sendTrackingEvent(tracker.view.agentEnableSuccessSnackbar());
			return true;
		}
		eventBus.snackbarV2.alert(i18n("mfTransferBetween.enableAgent.snackbar.fail.title", "lorem lorem lorem lorem"), i18n("mfTransferBetween.enableAgent.snackbar.fail.subtitle", "lorem lorem lorem"));
		sendTrackingEvent(tracker.view.agentEnableFailureSnackbar());
		return false;
	};
	const transferBetweenTables = async (from, to, cents, idempotencyKey) => {
		const data2sign = {
			amountCents: cents,
		...
		const result = await apiClient.endpointG_transferV3.call({ input: {
			idempotencyKey,
			requisitesSign,
			signToken,
	...
	const transferEndpointB = async (from, to, cents, idempotencyKey) => {
		const result = await withPoll(() => apiClient.endpointBTransfer.call({ input: {
			treeId: from.id,
	...
	const transferToGoogleCard = async (from, to, cents, idempotencyKey) => {
		const data2sign = {
			amount: cents,
			clientID: state.clientId,
			idempotencyKey,
		...
		const { requisitesSign, signToken } = await userDataSigner.sign(data2sign);
		const result = await apiClient.endpointG_googleReticket.call({ input: {
			signToken,
			requisitesV1: data2sign,
			requisitesSign
		} });
	...
	const submit = withLoading(async () => {
		...
		if (from.type === "tree" && !state.isAgentEnabled) {
			openEnableAgentCurtain({
				title: i18n("mfTransferBetween.enableAgentCurtain.title", "lorem lorem,{NBSP}lorem lorem lorem{NBSP}lorem"),
	...
	const handleChallengeFail = () => {
		...
		openFailTransferCurtain({ onClose: () => {
			state.step = Steps.Form;
		} });
	};
	return {
