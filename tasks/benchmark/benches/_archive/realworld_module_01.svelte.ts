...
import {
	ProductTypeA,
	AlphaTransferDisplayAction,
	Card2TableTransferFromInput as TransferFrom,
...
import type { TreeCardType, FinePayload, ErrorConfig, Step } from '$shared/apps/MFAlpha/types';
import { type AlphaIncomingUrlParams, alphaNavigation } from '$shared/navigation/c2a';

import type { Contract } from './contract';
import { openSuccessC2ACurtain } from './curtains/successC2ACurtain';
import { mapState } from './mappers';

function makeStore(contract: Contract) {
	const commissionCalculator = new CommissionCalculator();
	let step = $state<Step>('form');
	let selectedTreeCard = $state<TreeCardType | undefined>();
	let error = $state<ErrorConfig | undefined>();

	const state = lazy(null, async () => {
		const incomingUrlParams = alphaNavigation.extractIncomingUrlParam(window.location.search);
		// lorem lorem lorem lorem, lorem, lorem lorem lorem lorem lorem 100 lorem
	...
	/**
	 *  lorem lorem lorem lorem lorem lorem endpointB
	 *  lorem lorem lorem endpointB lorem tableId lorem lorem lorem lorem lorem lorem
	 *  lorem lorem lorem lorem lorem - lorem lorem lorem lorem endpointB
	 */
	const endpointBTableId = $derived.by(() => {
		const currentTable = state.data?.currentTable;

		if (
			currentTable?.type === ProductTypeA.ACCOUNT_TYPE_A ||
			currentTable?.type === ProductTypeA.ACCOUNT_TYPE_B ||
			currentTable?.type === ProductTypeA.ACCOUNT_TYPE_C ||
			currentTable?.type === ProductTypeA.ACCOUNT_TYPE_D
		) {
			return currentTable.number;
		}
	});

	function next(inStep: Step) {
		step = inStep;
	}
	...
	const limitStrategy =
		contract.v1?.limitStrategy ??
		new DefaultLimitStrategy(() => ({
	...
	async function createDeposit(payload: FinePayload): Promise<void> {
		...
		if (!currentTable) {
			eventBus.snackbarV2.alert(
				i18n(
					'mfalpha.createDeposit.error.currentTableNull',
		...
		const result = await apiClient.endpointA_createV2.call({
			input: {
				amount: payload.amount,
				failUrl: returnUrl,
				successUrl: returnUrl,
				// lorem: lorem lorem lorem lorem lorem lorem-lorem - lorem lorem lorem lorem
				// transferFrom - lorem lorem lorem
				transferFrom: contract.v1?.topDisclaimer
					? TransferFrom.PPR_CHECKOUT
					: TransferFrom.TREE_CABINET,
				// unreachable - lorem lorem validator lorem lorem
		...
		if (result.isFail()) {
			return eventBus.snackbarV2.alert(
				i18n('mfalpha.createDeposit.error.operationFail', 'lorem lorem lorem lorem')
			);
		}
	...
	const pollStatus = withPoll(apiClient.endpointA_statusV2, {
		validate: (result) => {
			...
			if (result.value.displayAction) {
				if (result.value.displayAction === AlphaTransferDisplayAction.ALPHA_DISPLAY_ACTION_PROCESSING)
					return false;
				return true;
			}
			return false;
	...
	async function notifyTransferSuccess({
		transferAmt,
		cardMask,
	}: {
		transferAmt: number;
	...
	async function verifyTransferResult(params: AlphaIncomingUrlParams): Promise<void> {
		...
		if (pollingResult.isFail()) {
			...
			eventBus.snackbarV2.alert(
				i18n('mfalpha.verifyTransfer.error.transferFail', 'lorem lorem lorem lorem')
			);
			return;
		}
		...
		if (
			result.value.displayAction === AlphaTransferDisplayAction.ALPHA_DISPLAY_ACTION_SUCCESS &&
			transfer
		) {
	...
	function selectTreeCard(selectedToken?: string): void {
