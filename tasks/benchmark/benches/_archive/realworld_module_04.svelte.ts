...
import {
	Error,
	type PerformTransferByCardNumberRequisitesV2Input,
} from '$gatewayService/__queryTypes';
import {
	CardTranferHint,
	CardTransferType,
	GroupOperationTypeV2,
	FlowProcessFailedReason,
	TicketTemplateType,
} from '$gatewayService/__queryTypes';
...
import type { AmountValidation, Commission, State } from '../types';

function makeStore(initState: State) {
	...
	function updateCardNumberError(message: string) {
	...
	async function changeTicketSystemByBin(bin: string, isThemeDark: boolean) {
		const system = await iframeMethod.changeTicketSystemByBin(bin, isThemeDark);
		state.ticketSystem = system;
	}

	const setSelectedTable = (key: string) => {
	...
	async function updateAmount(inAmount: number | undefined) {
	...
	async function setupCardTransfer(clientCardId: string, inMaskedPan: string) {
		const fail = (message: string) => {
			updateCardNumberError(message);
			state.commission = { status: 'notcalculated', value: 0 };
			return failResult('any error');
		};

		const result = await apiClient.endpointD_initDomesticTransfer.call({
			input: {
		...
		if (result.isFail()) {
			...
			if (
				errorMessage.includes(Error.CARD_TRANSFER_ERROR) ||
				errorMessage.includes(Error.TEMPORARILY_UNABLE_TO_TRANSFER_TO_SPECIFIED_PPR_CARD)
			) {
		...
		if (localTransferType === CardTransferType.UNAVAILABLE_TRANSFER) {
			...
			return fail(
				i18n(
					'endpointD.initTransfer.error.unavailableTransfer',
					'lorem lorem lorem lorem lorem lorem'
				)
			);
		}

		// lorem lorem payoffId lorem lorem lorem lorem lorem
		if (!result.value.payoffId) {
			return fail(
				i18n(
					'endpointD.initTransfer.error.payoffId.isBlank',
		...
		if (
			result.value.hints.includes(
				CardTranferHint.HINT_TRANSFER_WILL_BE_SENT_TO_MAIN_RECIPIENT_TABLE
			)
		) {
			iframeMethod.setCaption(
				i18n(
					'endpointD.cardNumberIframe.transferHint',
					'lorem lorem lorem {tableName} lorem',
	...
	async function loadCommission(amount: number | undefined): Promise<Commission> {
		if (
			isNil(amount) ||
			!state.recipientTransferInfo?.payoffId ||
			!state.recipientTransferInfo?.transferType
		) {
			return { status: 'notcalculated', value: 0 };
		}

		const result = await apiClient.endpointD_prepareDomesticTransfer.call({
			input: {
				amount: { currencyCode: '111', cents: amount },
		...
		if (result.isFail()) {
			return { status: 'fail', value: 0 };
		}

		if (state.recipientTransferInfo?.transferType === CardTransferType.INTERNAL_TRANSFER) {
			return { status: 'ok', value: 0 };
		}
		return { status: 'ok', value: result?.value?.clientCommission?.sum ?? 0 };
	}

	function validateAmount(): AmountValidation {
	...
	async function getOperationOrDefault(groupOperationId: string, transferType: CardTransferType) {
		const result = await apiClient.endpointD_operationSummary.call({
			operationType:
				transferType === CardTransferType.INTERNAL_TRANSFER
					? GroupOperationTypeV2.C_2_C_BY_CARD_NUMBER_INTERNAL_OUTGOING
					: GroupOperationTypeV2.TRANSFER_BY_CARD_NUMBER_OUTGOING,
			groupOperationId,
		});
	...
	async function performTransferSigned(
		amountCents: number,
		payoffId: string,
		transferType: CardTransferType
	) {
		const data2Sign: PerformTransferByCardNumberRequisitesV2Input = {
		...
		return await apiClient.endpointD_performTransferV3.call({
			input: {
				signToken,
				requisitesSign,
				payoffId,
	...
	async function performTransfer() {
		...
		if (!payoffId) {
			updateCardNumberError(
				i18n(
					'endpointD.fineTransfer.error.payoffId.isBlank',
		...
		if (isNil(sum)) {
			state.amountError = i18n(
				'endpointD.fineTransfer.error.sum.isBlank',
				'lorem lorem lorem lorem'
			);
		}
		...
		if (!isCommissionValid) {
			...
			eventBus.snackbarV2.info(
				i18n('endpointD.formView.snackbar.commissionLoading.title', 'lorem lorem'),
				i18n(
					'endpointD.formView.snackbar.commissionLoading.description',
					'lorem lorem lorem lorem'
				)
			);
		...
		if (fineResult.isFail()) {
			eventBus.snackbarV2.alert(
				i18n('endpointD.fineTransfer.error.transferFail', 'lorem lorem lorem lorem')
			);
			return 'error';
		}
		...
		const needTransferApprove =
			payoff.failedReason ===
			FlowProcessFailedReason.REASON_ONLINE_CONTROL_DECLINED_APPROVE_REQUIRED;
		...
		state.operationSummaryData = {
			...
			commission: operation.commission,
			title:
				needTransferApprove && approverId
					? i18n('endpointD.fineTransfer.needTransferApprove', 'lorem lorem')
					: undefined,
