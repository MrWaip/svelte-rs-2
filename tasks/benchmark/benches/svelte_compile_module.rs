...
fn bench_svelte_compile_module(criterion: &mut Criterion) {
    let files = glob("./benches/compiler/**/*.svelte.js").expect("Не удалось считать модули");

    let mut group = criterion.benchmark_group("compile_module");
...
criterion_group!(compile_module, bench_svelte_compile_module);
criterion_main!(compile_module);
