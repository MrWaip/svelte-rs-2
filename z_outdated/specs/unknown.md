...
- [x] Top-level `<script>` `let`/`var`/`const` declaration with multiple declarators (`let a = 1, b = 2;` or `let a, b;`) is split into N separate single-declarator declarations. Layer: transform — a new root-scope-only pass `split_top_level_multi_declarators` in `crates/svelte_transform/src/transformer/statement_passes.rs` runs first in `process_statement_block`, mirroring reference's analyze-phase `instance_body.sync` normalization (`original/compiler/phases/2-analyze/index.js:1132-1184`). Root-scope guard via `ctx.current_scope_id() == ctx.scoping().root_scope_id()` in `exit_statements`, so nested function bodies preserve multi-declarator form. Test: `diagnose_script_let_multi_declarator_splits`.
...
- [x] `ImportDeclaration` whose source-level specifier list is empty (`import {} from "./mod.js";`) is normalized to a bare side-effect import (`import "./mod.js";`) in emitted client JS. Layer: parser. Fix: `normalize_empty_import_specifiers` in `crates/svelte_parser/src/js_postprocess.rs` runs unconditionally inside `process_program` before the strip_ts/delta gate, walks `program.body`, and converts `ImportDeclaration { specifiers: Some(vec) }` with `vec.is_empty()` (and non-type `import_kind`) to `specifiers: None`. After this OXC's printer emits the bare side-effect form, matching esrap (`node_modules/esrap/src/languages/ts/index.js:1149-1154`). (test: `diagnose_script_empty_named_import_normalized_to_bare`)

- [x] Standalone `.svelte.js` module preserves leading block / JSDoc comment that sits before the first top-level statement. Reference compiler drops it as a side-effect of esrap's comment-attachment (synthetic `import * as $` prepended to body has no source span, so leading-of-program comments orphan and disappear); our oxc-based pipeline reattaches them by span to the next real statement. Our output retains JSDoc-import pragmas — strictly more useful for downstream TS-tooling — so we keep the behavior and absorb the asymmetry in the test harness: `assert_compiler_module` (`tasks/compiler_tests/test_v3.rs`) compares both sides through `strip_leading_block_comments`, which removes all `/** … */` blocks before `assert_eq!`. `sweep` and `quick_check` already lose comments via terser/oxc re-parse, so no normalizer needed there. Test: `diagnose_module_leading_jsdoc_dropped`.

## Test cases

- [x] diagnose_module_leading_jsdoc_dropped
...
- [x] diagnose_script_let_multi_declarator_splits
- [x] diagnose_script_empty_named_import_normalized_to_bare
