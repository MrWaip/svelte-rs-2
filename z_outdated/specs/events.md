...
- [x] Load-error elements (`<body>`, `<embed>`, `<iframe>`, `<img>`, `<link>`, `<object>`, `<script>`, `<style>`, `<track>` — canonical `LOAD_ERROR_ELEMENTS` from `svelte/src/utils.js`) emit `$.replay_events(<node>)` into `after_update` when they carry spread, `use:`, `onload`, or `onerror`. Elements outside this list (notably `<source>`) MUST NOT emit `$.replay_events` even with spread (tests: `img_spread_replay_events`, `embed_spread_replay_events`, `object_spread_replay_events`, `source_spread_in_each_no_replay_events`)
...
- [x] `attribute_invalid_event_handler` is scoped to `RegularElement` and `<svelte:element>` only, matching reference's `validate_element` in `original/compiler/phases/2-analyze/visitors/shared/element.js`. Component-family visitors (`visit_component_node`, `visit_svelte_component_legacy`, `visit_svelte_self`) no longer invoke `template_validation::check_event_handler_value`, so an `on`-prefixed boolean/string attribute on a component (e.g. `<Foo onlyOrder />`) is treated as an ordinary prop. Owning layer: 3.C `validate`. (test: `diagnose_component_attr_on_prefix_false_positive`)
...
- [x] `on:<event>={ident?.member}` whose handler resolves to a `HandlerEmit::WrappedInert` binding (e.g. `export let store = undefined; <button on:click={store?.reset}>`) must emit the apply-wrapper as `function(...$$args) { store()?.reset?.apply(this, $$args); }` — without parenthesising the optional-chain expression. Today our codegen wraps the resolved expression in extra parens: `(store()?.reset)?.apply(this, $$args);`. Reference passes the optional member through bare. Layer: codegen — handler-emit serializer for `WrappedInert` (`crates/svelte_codegen_client/src/template/events/handlers.rs`) must not parenthesise an `OptionalMemberExpression`/`OptionalChainExpression` callee before suffixing `?.apply(this, $$args)`. Test: `diagnose_on_directive_optional_chain_handler_wrap`.

## Reference

- Reference analyze:
  - `original/compiler/phases/2-analyze/visitors/Attribute.js`
  - `original/compiler/phases/2-analyze/visitors/OnDirective.js`
  - `original/compiler/phases/2-analyze/visitors/shared/element.js`
...
- Reference client transform:
  - `original/compiler/phases/3-transform/client/visitors/OnDirective.js`
  - `original/compiler/phases/3-transform/client/visitors/shared/events.js`
...
- [x] `embed_spread_replay_events`
- [x] `object_spread_replay_events`
- [x] `source_spread_in_each_no_replay_events`
...
- [x] `diagnose_on_directive_optional_chain_handler_wrap`
