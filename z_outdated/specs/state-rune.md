...
- [x] Private class field declared without initializer (`#x;`) then assigned `$state(...)` / `$state.raw(...)` in the constructor is recognized as reactive: rune call lowers to `$.state(...)`, subsequent reads → `$.get(this.#x)`, writes → `$.set(this.#x, ...)` (covered, test: `diagnose_state_private_constructor_init_from_decl_only`)
- [x] Private class field with non-literal `$state(...)` initializer wraps argument in `$.proxy(...)`: `#items = $state([])` → `#items = $.state($.proxy([]))` (covered, test: `state_private_class_field_array_proxy`)
...
## Reference
- Svelte reference:
  - `original/compiler/phases/2-analyze/visitors/VariableDeclarator.js` — binding.kind assignment
  - `original/compiler/phases/2-analyze/visitors/CallExpression.js` — placement validation
  - `original/compiler/phases/2-analyze/visitors/Identifier.js` — deprecation/removal errors
  - `original/compiler/phases/3-transform/client/visitors/CallExpression.js` — $.state/$.proxy generation
  - `original/compiler/phases/3-transform/client/visitors/VariableDeclaration.js` — variable wrapping, destructuring
  - `original/compiler/phases/3-transform/client/visitors/ClassBody.js` — class field transforms
  - `original/compiler/phases/3-transform/client/visitors/MemberExpression.js` — private field reads
  - `original/compiler/phases/3-transform/client/visitors/AssignmentExpression.js` — $.set generation
...
- [x] `diagnose_state_private_constructor_init_from_decl_only`
- [x] `state_private_class_field_array_proxy`
- [x] `hello_state`
- [x] `state_runes`
- [x] `state_raw`
