...
- `original/compiler/phases/1-parse/state/tag.js` — `{#await}`, `{:then}`, `{:catch}` parsing
- `original/compiler/phases/2-analyze/visitors/AwaitBlock.js` — validation, `mark_subtree_dynamic`
- `original/compiler/phases/3-transform/client/visitors/AwaitBlock.js` — `$.await()` codegen, `create_derived_block_argument`
- `original/compiler/types/template.d.ts` — `AwaitBlock` AST type definition
