...
- [x] `bind:files` on `<input type="file">` whose target is a legacy `export let` prop (`$.prop(...)` accessor): reference passes the accessor identifier directly to `$.bind_files(input, files)` (single-arg shorthand). We currently panic in codegen with `bind without getter/setter must be bind:this` because the transform bindable-prop short-circuit drops the getter/setter and codegen lacks a `Files` shorthand branch alongside `Checked`. Owner: codegen `crates/svelte_codegen_client/src/codegen/attributes/bind/mod.rs::emit_bind_bindable_prop_shorthand` — extend to emit `$.bind_files` single-arg form for `ElementBindPropertyKind::Files`. (test: `diagnose_legacy_bind_files_export_let_prop`)
...
- [x] Legacy `<input type="radio">` with `bind:group={model}` plus a sibling `{value}` shorthand attribute and `class:` directive sequences attribute updates in reference order inside `$.template_effect`: independent attribute updates (`set_attribute`, `set_class`, `el.<prop> = …`) precede the value-sync `if (input_value !== (input_value = value())) { … }` block, and declarations are `let classes;` then `var input_value;`. Owner: codegen `emit_dom_attributes_with_kind` in `crates/svelte_codegen_client/src/codegen/attributes/dispatch.rs` — when an `ExpressionAttribute` named `value` on an `<input>` with `bind:group` is encountered in the main loop, it's stashed and the underlying `emit_attr_expression` call is deferred until after the class-directive fallback runs, mirroring reference's `build_element_special_value_attribute` deferral at the end of `RegularElement.js` per-element processing. (test: `diagnose_legacy_bind_group_radio_attr_update_order`)
- [x] Element `bind:group` value-cache for an `<input>` whose value expression is statically known to be defined (`Evaluation::Known(_)` / `Evaluation::Defined { .. }`) omits the `?? ""` coalesce on the `el.value = (el.__value = expr)` assignment. Reference: `build_element_special_value_attribute` branches on `evaluated.is_defined`. Owner: codegen `emit_bind_group_value` in `crates/svelte_codegen_client/src/codegen/attributes/bind/group.rs` consults the existing `evaluation_is_defined(&data.evaluation)` predicate (already used by `regular.rs`, `expression_prop.rs`, `title.rs`) on the `ExpressionData` returned by `Ctx::expression_data(attr_id)`; branches `value_rhs` between the bare `(el.__value = expr)` assign and the `?? ""` coalesce. (test: `bind_group_value_defined`)
- [x] `<option value={expr}>` value-cache (`emit_option_expr_value` in `crates/svelte_codegen_client/src/codegen/attributes/option_value.rs`) gates the `?? ""` coalesce on `evaluation_is_defined(&data.evaluation)` from `ExpressionData` — when the value expression has `Evaluation::Known`/`Evaluation::Defined` (e.g. template-literal / typed-defined), emits `el.value = el.__value = expr` (chained assignment, no fallback), mirroring reference's `build_element_special_value_attribute`. Caller in `expression_attr.rs` threads `attr.id` through; local `OptionValueKind` enum collapsed into a `needs_coalesce: bool` parameter shared with `emit_option_concat_value` (concatenations are always String-defined). (test: `option_expr_value_defined`)
- [x] Legacy `<input>` with `bind:group={model}`, a sibling `{value}` shorthand attribute, and a `use:action` directive emits `var input_value;` AFTER the deferred `$.effect(() => $.bind_group(...))` and `$.action(input, ...)` calls (immediately before the `$.template_effect` that consumes the cache), not eagerly after `let classes;`. Reference places the value-cache declaration adjacent to the `template_effect` that reads it; when a `use:` action defers events/binds into `$.effect(...)` wrappers, the cache var declaration moves with them. Owned by codegen: `EmitState` gained a new element-scoped `pending_pre_update: Vec<Statement<'a>>` buffer saved/restored around `emit_dom_attributes` in `crates/svelte_codegen_client/src/codegen/containers/element.rs` and drained into `state.init` immediately after `my_element_init`, so the cache decl lands AFTER deferred bind/use init statements but BEFORE any sibling element's `$.reset(parent)` push. `emit_bind_group_value` in `crates/svelte_codegen_client/src/codegen/attributes/bind/group.rs` pushes `var <el>_value;` into `pending_pre_update` instead of `state.init`. (test: `diagnose_legacy_bind_group_value_use_action_cache_var_order`)
- [x] `<option value={expr}>` with a `use:action` directive (or any other `pending_element_init` producer on the same `<option>`) emits the `var <el>_value = {};` cache declaration AFTER the deferred `$.effect(...)` / `$.action(...)` calls, just before its consuming `$.template_effect(...)`. Owned by codegen `emit_option_special_value` in `crates/svelte_codegen_client/src/codegen/attributes/option_value.rs`: cache decl is pushed into `state.pending_pre_update` (drained into `state.init` after `my_element_init` by `containers/element.rs`), mirroring the `bind:group` fix in `bind/group.rs`. Covers both `<option value={expr}>` (`emit_option_expr_value`) and `<option value="prefix-{expr}">` (`emit_option_concat_value`) via the shared helper. (test: `option_expr_value_use_action_cache_var_order`)
- [x] Legacy `<input type="radio" bind:group {value} {...$$restProps}>` keeps the `value` expression inside the `$.bind_group` getter-callback's val_stmt. When a spread attribute coexists on the element, attributes flow through `$.attribute_effect`, which destructively pulls the value-attribute expression via `parsed.take_expr` before `emit_bind_group` clones it back through `parsed.expr(found_id)`. Owned by codegen `emit_attr_spread_full` in `crates/svelte_codegen_client/src/codegen/attributes/spread_attr.rs`: the `Attribute::ExpressionAttribute` branch checks `AttributeSemantics::SpecialValueAttr { kind: SpecialValueKind::InputBindGroup }` and applies the existing `clone → take → replace_expr` backup pattern (mirroring `expression_attr.rs::emit_attr_expression` for the non-spread path) so the value-attr OXC expression survives spread consumption and is available to `emit_bind_group` later in the same call. (test: `diagnose_legacy_bind_group_radio_value_shorthand_spread`)
- [x] Legacy `<input type="radio" bind:group={model} value={index}>` inside `{#each ... as _, index}` where the `value` expression has no reactive dependencies emits `input.value = input.__value = index;` directly in init — no cache var, no `$.template_effect` guard. Owner: codegen `emit_bind_group_value` in `crates/svelte_codegen_client/src/codegen/attributes/bind/group.rs` consults `ExpressionData.kind` from `self.ctx.expression_data(attr_id)`: when `kind` is not `SimpleRead { reactive: true } | Computed { reactive: true } | Call { .. } | Async { .. }` (i.e., `KnownLiteral`, `SimpleRead { reactive: false }`, `Computed { reactive: false }`), pushes the bare chained `value`/`__value` assignment into `state.init` (subject to the existing `is_defined` `?? ""` branching) and skips both the `pending_pre_update` cache decl and the `state.update` guarded if-block, mirroring reference `build_element_special_value_attribute`'s `has_state === false` branch in `original/compiler/phases/3-transform/client/visitors/RegularElement.js:708-720`. Test: `diagnose_legacy_each_bind_group_radio_value_index_no_cache`.

## Reference

- `original/compiler/phases/bindings.js` — canonical binding property matrix
- `original/compiler/phases/2-analyze/visitors/BindDirective.js` — analyzer validation and group-binding metadata rules
- `original/compiler/errors.js` — bind and attribute diagnostic definitions
- `original/compiler/warnings.js` — `bind_invalid_each_rest`
- `original/compiler/phases/3-transform/client/visitors/BindDirective.js` — reference client transform surface
- `original/compiler/phases/3-transform/client/visitors/shared/component.js` — `$$ownership_validator.binding(...)` for component bindings
...
- [x] `bind_files`
- [x] `diagnose_legacy_bind_files_export_let_prop`
- [x] `bind_focused`
...
- [x] `diagnose_legacy_bind_group_radio_attr_update_order`
- [x] `diagnose_legacy_bind_group_value_use_action_cache_var_order`
- [x] `diagnose_legacy_bind_group_radio_value_shorthand_spread`
- [x] `diagnose_legacy_each_bind_group_radio_value_index_no_cache`
...
- [x] `bind_group_value_defined`
- [x] `option_expr_value_defined`
- [x] `option_expr_value_use_action_cache_var_order`
