...
- `original/compiler/warnings.js` — 81 warning codes, parameterized messages, `w()` function
- `original/compiler/errors.js` — ~165 semantic error codes, parameterized messages
- `original/compiler/state.js` — ignore_stack, ignore_map, push/pop/is_ignored, warning_filter
- `original/compiler/utils/extract_svelte_ignore.js` — comment parsing (runes vs legacy mode)
- `original/compiler/phases/2-analyze/index.js` — ignore integration in `_` visitor
