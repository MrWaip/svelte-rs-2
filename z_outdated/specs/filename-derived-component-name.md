...
### Svelte
- `original/compiler/phases/2-analyze/index.js` - `get_component_name(filename)` and `module.scope.generate(options.name ?? component_name)`
- `original/compiler/phases/scope.js` - `Scope.generate(preferred_name)` sanitization and conflict avoidance
- `original/compiler/state.js` - `rootDir` adjustment applies to runtime `filename`, not the component-name derivation path
- `original/compiler/validate-options.js` - `filename`, `rootDir`, and `name` option validation/defaults
