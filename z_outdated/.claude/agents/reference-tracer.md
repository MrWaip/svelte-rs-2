...
All reference compiler source is in `original/compiler/`. Trace the feature through all 3 phases:

1. **Parse** (`original/compiler/phases/1-parse/`) — syntax variants, how the feature is parsed
2. **Analyze** (`original/compiler/phases/2-analyze/visitors/`) — metadata, flags, special conditions
3. **Transform** (`original/compiler/phases/3-transform/client/visitors/`) — codegen branches, runtime calls

Also check:
- `original/compiler/types/template.d.ts` — AST node shape, optional fields, union variants
- `original/compiler/warnings.js` — related warning codes
- `original/compiler/tests/` — snapshot inputs and expected outputs
