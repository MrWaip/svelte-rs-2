...
- **Current slice**: re-audit dynamic attribute matcher/value expansion before implementation. Static string attrs are done, but the first attempted follow-up showed that even known-literal quoted concatenations remain conservative in the reference compiler.
- **Why this slice next**: the static operator semantics are verified, but the remaining runtime-shaped cases need reference-by-reference auditing before changing `css_prune`; otherwise we risk “improving” scope injection or `css_unused_selector` beyond upstream behavior.
- **Non-goals for this run**: CSS comment preservation, sibling/pseudo/boundary selector expansion unrelated to attribute values, and explicit `css: "external"` option cleanup.
...
- [ ] Unused selector warning (`css_unused_selector`) — basic type/class/ID matching with descendant/child combinators works; missing: sibling combinators, `:is/:where/:not/:has` special matching, nesting selector, dynamic attribute value matching, component/snippet boundary matching
