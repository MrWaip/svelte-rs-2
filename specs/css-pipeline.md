...
- **Non-goals for this run**: nesting selector matching, dynamic attribute value expansion, component/snippet boundary matching, CSS comment preservation, and explicit `css: "external"` option cleanup.
...
- [ ] Unused selector warning (`css_unused_selector`) — basic type/class/ID matching with descendant/child/sibling combinators plus `:is(...)` / `:where(...)` works; missing: `:not(...)` / `:has(...)` special matching, nesting selector, dynamic attribute value matching, component/snippet boundary matching
