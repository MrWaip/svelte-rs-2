...
- **Missing**: 3 use cases — role-transition warnings, scoped `svelte-ignore` suppression for `A11yNoStaticElementInteractions`, and the element-content A11y cluster
- **Next**: implement scoped `svelte-ignore` suppression for `A11yNoStaticElementInteractions` first (diagnostic parity case `a11y_no_static_element_interactions_ignored_on_mouseenter_mouseleave`), then role-transition warnings and the element-content A11y cluster
