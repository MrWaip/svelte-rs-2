...
### Svelte
- `reference/compiler/index.js` — `compile`, `compileModule` entry points; result assembly
- `reference/compiler/types/index.d.ts` — `CompileResult`, `CompileOptions`, `SourceMap` public types
- `reference/compiler/validate-options.js` — line 78 `cssOutputFilename`, line 102 `outputFilename`, line 120 `enableSourcemap` warn-removed
- `reference/compiler/warnings.js` — line 569-571 `options_removed_enable_sourcemap`
- `reference/compiler/phases/3-transform/index.js` — line 37 `get_source_name` for js, lines 39-44 esrap `print` with `sourceMapContent`/`sourceMapSource`, lines 88-107 `transform_module` banner + `';'` mapping prefix
- `reference/compiler/phases/3-transform/css/index.js` — lines 29-73 `render_stylesheet`, lines 56-63 `MagicString.generateMap`, lines 68-70 dev inline sourceMappingURL
- `reference/compiler/phases/3-transform/client/visitors/shared/fragment.js` — line 118 `b.id(name, node.name_loc)` element-identifier span
- `reference/compiler/phases/3-transform/client/transform-client.js` — line 404 `component_block.loc = instance.loc`
- `reference/compiler/utils/builders.js` — line 268-273 `b.id(name, loc)` accepting optional `SourceLocation`
- `reference/compiler/utils/mapped_code.js` — `get_source_name` (452-454), `get_basename` (443-445), `get_relative_path` (423-437)
