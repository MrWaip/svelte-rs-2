...
- [x] Define query inventory and map each query to one sanctioned structure:
  - Attribute presence / value / class / id / runtime queries -> `ElementFacts`
  - Parent / ancestor / sibling / nearest-element queries -> `TemplateTopology`
  - Direct-child shape queries -> `FragmentFacts`
  - CSS candidate search -> `TemplateElementIndex`
  - Each-block ownership queries -> `EachContextIndex`
- [x] Introduce new data modules under `crates/svelte_analyze/src/types/data/` for the sanctioned structures and expose them via `AnalysisData`
- [x] Refactor `ElementFlags` so it no longer mixes shared normalized facts with downstream pass-specific decisions
- [x] Build the query/index layer in one early analyze step, likely adjacent to or folded into `template_side_tables`
- [x] Add accessor methods on `AnalysisData` and `CodegenView` so consumers do not depend on internal storage layout
- [x] Migrate consumers in bounded slices:
  - Slice 1: `ElementFacts` + `TemplateTopology`, migrate `reactivity` and `content_types`
  - Slice 2: migrate `template_validation`
  - Slice 3: migrate `bind_semantics`
  - Slice 4: `FragmentFacts`, migrate repeated child-shape scans
  - Slice 5: `TemplateElementIndex`, migrate CSS analyze/prune
  - Slice 6: `EachContextIndex`, migrate each-block ownership queries and remove leftover ancestor/scope glue
  - Slice 7: narrow `CodegenView` to sanctioned query-layer accessors for remaining low-level reads
- [x] Add analyzer unit tests for each query structure's semantics and for migrated consumer behavior
- [x] Establish and document a rule for future analyze work: no pass-local query indexes when a sanctioned query structure can own the data
- [x] Establish and document a rule for future attr queries: consumer passes do not call `attr_index(...)` directly and instead extend `AnalysisData` / `CodegenView` with domain-shaped helpers when needed
- [x] Close this spec once all bounded migration slices are complete
- Future work, if any, should be tracked in a new spec instead of as unfinished debt here:
  - broader CSS selector completeness beyond concrete recorded-element queries

## Implementation order

1. Introduce `ElementFacts` and `TemplateTopology`
2. Migrate `reactivity` and `content_types`
3. Migrate `template_validation`
4. Migrate `bind_semantics`
5. Introduce `FragmentFacts`
6. Introduce `TemplateElementIndex` and migrate CSS
7. Expand `EachBlockData` into `EachContextIndex`-style API or add a dedicated structure
8. Clean up leftover pass-local glue and narrow `CodegenView` to sanctioned accessors

## Discovered bugs

- FIXED: `css_prune.rs` no longer builds a pass-local `TemplateElement` model or rescans attributes for class/id checks; the CSS passes now read shared analyze query data through `TemplateElementIndex`
- FIXED: each-block ownership and bind/every-context lookups no longer live in pass-local ancestor/scope glue or split low-level tables; `EachContextIndex` now owns the sanctioned query surface
