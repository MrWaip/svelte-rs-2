...
- Run `just generate` to generate `case-svelte.js` (expected output from Svelte v5). If this fails, stop and report the error.
- **NEVER edit `case-svelte.js` or `case-rust.js` files.** `case-svelte.js` is generated by the Svelte v5 compiler (ground truth). `case-rust.js` is written by our compiler during the test run. Both are codegen outputs — modifying them manually defeats the purpose of the test. Only `case.svelte` is hand-written.
- Add test function in `tasks/compiler_tests/test_v3.rs`: `#[rstest] fn <test_name>() { assert_compiler("<test_name>"); }`
