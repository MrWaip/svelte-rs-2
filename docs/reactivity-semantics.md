...
- `summary() -> ReactivitySummary { props, has_store_bindings, has_runes_bindable, has_legacy_reactive_statements, has_named_runes_prop, has_named_legacy_prop, legacy }` — единый агрегат компонент-уровневых фактов реактивности, чтобы потребитель читал вердикт одним запросом, а не пересобирал его сканом `bindings`. `props` — runes-props (`has_props` / `has_bindable` / `has_custom_element`); `legacy` — `has_bindable_prop`, `reads_props_object` (`$$props`), `reads_rest_props_object` (`$$restProps`), `has_member_mutated`, `reads_slots_object` (`$$slots`); верхний уровень — оси наличия stably-bindable рун-пропа, `$:`-блока и публично-именованного (не-`$$`) пропа. Потребители — `RuntimeSemantics` и `CodegenView`. `has_bindable` — отдельный флаг, а не скан `bindings`: оптимизация в `finish()` стирает bindable-метку у read-only source-props.
...
- `iter_legacy_bindable_prop_symbols() -> impl Iterator<Item = SymbolId>` — bindable-props legacy-режима (`export let` и reassignable `export { local as name }`, LEGACY(svelte4)) в порядке объявления, поверх `BindingFacts::LegacyBindableProp`. Порядок — сам порядок объявления символов (форк Оригинала: итерация `scope.declarations`), поэтому потребители (excludes `legacy_rest_props`, accessor-пропы) не держат отдельный упорядоченный индекс. Компонент-уровневый флаг наличия — `summary().legacy.has_bindable_prop`.
...
- `each_item_collection_store(SymbolId) -> Option<SymbolId>` — если each-item итерируется по store-подписке (`{#each $store as item}`), возвращает символ store-коллекции. Потребитель — dev-обёртка `$.mark_store_binding` в object-thunk `$.validate_binding` (порт `store_to_invalidate` Оригинала): bind внутри store-коллекционного each помечает store-биндинг.

Синтаксическая bindability runes-пропа (`let { x = $bindable() } = $props()`) — **стабильная ось** `PropBindingSemantics.bindable`, независимая от оптимизации `PropBindingKind`: незаписанный `$bindable()` без default демотируется в `NonSource`, но `bindable` переживает демоцию. Читается тотальным `binding_semantics(sym).is_bindable()`; перечисление bindable-пропов для server-эмита `$.bind_props` — `iter_runes_prop_symbols().filter(is_bindable)`. Отдельный membership-индекс bindability рядом с вердиктом запрещён — он дублирует ось и десинхронизируется при демоции.
