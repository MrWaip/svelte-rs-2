...
2. **Read-only.** `JsVisitor` не мутируют ни AST, ни подсистемы — как и текущие валидаторы (принцип
   3.C).
...
## Статус: Фаза 1 поставлена

`js_walker.rs` — `JsVisitor` (хуки `enter_js_node` / `leave_js_node` по `AstKind`, возврат
`JsFlow::SkipSubtree` вместо «не звать `walk_*`») и драйвер `JsWalk`. На него переведены шесть
валидаторов инстанс-программы: `const_assignment`, `dollar_name`, `syntax_bundle`,
`experimental_async`, `runes::StateRefLocally`, `runes::RestPropAccess`. Module-программа и
standalone-модуль получили свои bundle'ы.

Две детали, без которых слияние проигрывает шести отдельным обходам:

1. **Маска интересов.** Специализированный `impl Visit` с пустыми дефолтами LLVM выбрасывает
   целиком — построение `AstKind` на узле не доходит до кода. Fan-out платит за `AstKind` и
   диспетч на каждом узле, поэтому визитор объявляет `enter_interests`/`leave_interests`
   (`JsNodeMask` по `AstType`), драйвер держит их объединение и на неинтересном виде узла выходит
   по одному тесту бита. Без масок слияние было **медленнее** исходных шести обходов (+3.5%).
2. **Отдельная маска на leave.** `IdentifierReference` интересен на входе почти всем и не нужен
   на выходе никому; общая маска на обе стороны съедала половину выигрыша.

Замер (46 компонентов реального приложения): −1% суммарно, −3% на script-heavy файлах,
+1% на мелких (плата за bundle на почти пустой программе). Доля `validate::validate` в профиле
`Modal.svelte` — 9.27% → 6.86%.

**Вывод для Фаз 2–3.** Стоимость обхода — не диспетчер, а работа на узле: полное снятие
валидации даёт −6.5%, слияние шести обходов из них — −1%. Оставшиеся фазы имеют тот же потолок и
приоритетом не являются; заметное ускорение компилятора пришло не отсюда, а из PGO
(`../adr/0007-pgo-release-build.md`, −12.5%).
