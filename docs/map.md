...
Пайплайн: **парсер → анализ → трансформ → кодген**, вход через `svelte_compiler`. JS/TS внутри `.svelte` — через OXC.
...
| Компиляция модуля (`.svelte.js`) | `compile_module` | `svelte_compiler/src/lib.rs:289` |
| Анализ | `analyze` / `analyze_module` | `svelte_analyze/src/lib.rs:126` |
| Трансформ | `transform_component` | `svelte_transform/src/lib.rs:25` |
| Кодген | `generate` / `generate_module` | `svelte_codegen_client/src/lib.rs:101` |
...
- `svelte_transform` — мутация JS AST. Вход `src/lib.rs`; пассы — `src/transformer/` (`runes.rs`, `state.rs`, `derived.rs`, `props.rs`, `legacy_reactive.rs`, `props_legacy.rs`, `assignments.rs`, `inspect.rs`, …).
