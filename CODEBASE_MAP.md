...
- `ElementFacts` владеет нормализованными per-element attribute facts и использует `AttrIndex` как внутренний by-name primitive. Consumer-pass’ы должны ходить через `AnalysisData` accessors (`has_attribute`, `attribute`, `string_attribute`, `bind_directive`, `expression_attribute`, `static_text_attribute_value`, etc.), а не вызывать `attr_index(...)` напрямую.
